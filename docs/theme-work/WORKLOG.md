...
## 2026-09-26 — все пять тем: «Логотип» в секции «Подвал» (`feat/footer-logo-0926`)

### Задача

Владелец: «Логотип меняется в хедере, а в подвале не могу — там стандартный
логотип темы. Добавь в футер такую же секцию, чтобы можно было там менять
логотип» (скриншот — поле загрузки «Добавить фото / Макс. размер 500 КБ /
Изменить» из «Настроек темы → Логотип»).

Понял так: отдельное поле загрузки в панели «Подвала» — своя картинка подвала,
тем же виджетом и с тем же пределом 500 КБ; пустое поле — подвал ровно как был.

### Замер до (прод, учётка design-parity, пять тем)

- панель «Подвала»: Рассылка / Заголовок / Размер заголовка / Текст / Размер
  текста / Цветовая схема / Отступы — логотипа нет ни в одной теме;
- «Настройки темы → Логотип»: загрузка 500 КБ + «Размер» — это логотип шапки;
- подвал на месте логотипа: vanilla — `/icons/Vanila-designers.svg` всегда
  (логотип шапки не берёт), satin — название магазина текстом (десктоп и
  телефон), bloom — логотип брендинга (`Footer.logo`, его кладёт сборка) или
  название, rose и flux — логотипа в вёрстке подвала нет.

### Правка

- `packages/theme-base/runtime/footer-logo.ts` — имя пропа `customLogo` и
  `footerOwnLogo(props)`; отдельный проп, потому что служебный `Footer.logo`
  перетирается брендингом при сборке;
- поле `customLogo: { type: 'image', label: 'Логотип', maxSizeKb: 500 }`
  первым в `Footer.puckConfig.ts` theme-base и satin (литералом: сборка блоков
  не переписывает импорт theme-base/runtime из пакета темы, а
  satin-sidebar-canon читает тип поля из исходника);
- порты `themes/<тема>/src/components/Footer.astro`: загружено — `<img>` на
  месте логотипа (vanilla вместо вордмарка, satin вместо текста в обоих
  вариантах, bloom раньше брендинга, rose/flux — над колонкой навигации), класс
  высоты `h-[var(--size-logo-width,…)]` как у картинки шапки той же темы;
- theme-base `Footer.astro` (общий блок) — тот же приоритет;
- конструктор: `ImageField` берёт предел из `maxSizeKb` поля (подпись и
  проверка из одного числа), без него — прежние 30 МБ.

### Проверки

- `footer-logo.spec.ts`: 31/31; на старом коде 20 красных (панель ×15, рендер ×10 − пусто ×5 зелёные);
- пустое поле: HTML подвала старого и нового кода совпадает байт-в-байт
  (5 тем × 4 набора пропов);
- `panel-canon` переснят (+5 полей), `satin-sidebar-canon`, `panel-field-defaults`,
  `hidden-named-fields`, `section-html-snapshot`, footer-* — зелёные (1179/1179);
- остальные красные полного прогона (golden, resolve-block-scheme, page-registry,
  debounce …) красные и на чистом `origin/main` — не наши;
- конструктор: `imageFieldSizeLimit.test.tsx` 7/7 (на старом 5 красных), весь
  vitest 640/640, `tsc --noEmit` чисто;
- пруф: прод-конструктор с локальной сборкой конструктора, puck-config и рендером
  «Подвала» — по пяти темам: поле «Логотип / Макс. размер 500 КБ», файл 626 КБ
  отклонён, загрузка 201, лого в подвале превью сразу и после перезагрузки,
  `customLogo` в ревизии. Снимки — scratchpad `footer-logo/`.

### Хвосты

- vanilla при пустом поле по-прежнему не берёт логотип шапки — решает владелец;
- инвентарь satin — пересчитать после коммита.
...
## 2026-09-23 — зазор между секциями без своей схемы

Ветка `fix/section-gap-unwrapped`. Повтор бага «отступы между секций берут
фон схемы 1» после #107.

- Замер сайта тестировщика (bloom, зазор 84 px): главная — промежутки цвета
  нижней секции; страница товара — белая полоса между «Товаром» и
  «Популярными» (секции без своей схемы, без обёртки).
- Причина: фон страницы у bloom/flux — жёсткий белый `body`, а секции без
...
## 2026-09-25 — Фикс: «Статичность → Всегда» у Шапки не применялась в превью (5 тем)

Ветка `fix/header-sticky-always-preview`, worktree `sites-sticky`. Баг воспроизводится только
если исходный режим шапки — «При прокрутке вверх» (дефолт у bloom и satin): скрипт темы
(`SCROLL_UP_JS`) вешал слушатель прокрутки один раз при первом рендере и не перечитывал
режим дальше, поэтому выбор «Всегда» в конструкторе менял CSS-классы шапки в превью, но при
прокрутке она всё равно уезжала `translateY(-100%)`.

- `src/services/preview.service.ts` — горячий патч `stickiness` в превью: классы старого режима
  снимаются поштучно (одна строка-регэксп не находила их при другом порядке классов у bloom/
  satin), атрибут `data-<тема>-sticky` переписывается на новый режим, `transform` сбрасывается.
- `themes/{rose,vanilla,flux,satin,bloom}/src/components/Header.astro` — `SCROLL_UP_JS`
  проверяет `data-<тема>-sticky` на каждой прокрутке и не прячет шапку, если режим уже не
  scroll-up.
- Новый тест `src/services/__tests__/preview-header-stickiness.spec.ts` — 20 проверок (5 тем ×
  4 сценария): на старом коде 10 падают, на новом 20/20.
- Инвентарь satin пересчитан (`conformance:satin:refresh-inventory`) — задет `sourceDigest`,
  структурные находки не изменились; `conformance:satin` проходит.
- Коммиты: `1e7065dd` (фикс), `e957bd1e` (инвентарь satin). PR в `main` репо
  `Merfy-Dropshipping-Platform/site-gen`.
...
## 2026-09-25 — перетаскивание параметров «Товара» (все пять тем)

- Владелец (25.09): вслед за «Изображением»/«Основным текстом»/«Подпиской» сделать то же для
  секции «Товар». Было: 8 параметров в дереве, 0 ручек — puck-config не объявлял `fieldOrder`.
- Сделано: `SECTION_FIELDS.Product` — все восемь строк дерева (Текст, Название, Стоимость,
  Варианты, Количество, Кнопки, Описание, Поделиться). Галерея фото в дереве не параметр — её
  сторону задаёт «Позиция фото», не переставляется. Скрытое `fieldOrder` в
  `Product.puckConfig.ts`; общий блок theme-base (`Product.astro` + `ProductInfo.astro`, рисует
  rose/vanilla/satin/bloom) и порт flux `FeaturedProduct.astro` (десктоп и мобила) рисуют строки
  в порядке `fieldOrder`. Соседние бренд/название/цена держатся в общей «шапке» (gap-2),
  остальные — звенья колонки.
- Помощник переехал `packages/theme-base/lib/field-order.ts` → `runtime/field-order.ts`:
  блоки theme-base при сборке видят только `runtime/` (compile-astro-blocks). Импорты портов
  переписаны, разметка секций не изменилась (отличается только хеш стилей Astro).
- Без `fieldOrder` разметка прежняя: общий блок — отличие только в пробелах между тегами; flux —
  плюс пропали служебные HTML-комментарии разработчика внутри колонки (стали JSX-комментариями).
- Гард: `src/themes/__tests__/product-field-order.spec.ts` (на старом коде 35 из 45 красные,
  с правкой 45 из 45) + шаг в ci.yml; `panel-canon.json` переснят (+5 скрытых `fieldOrder`).

## 2026-09-26 — кнопки ← поля цветовой схемы своей роли + стартовые цвета схем по вёрстке (вариант Б, все пять тем) — PR, не залито

- Владелец 25.09 выбрал «Б»: правило «поле схемы → одно свойство кнопки» (Фон → фон, Текст → буквы,
  Обводка → рамка; «Кнопки» = Основная + Дополнительная, «Кнопка» = всегда Основная) плюс
  стартовые значения схем тем подобраны так, чтобы новый магазин выглядел как сайт верстальщиков.
- Правило: `src/themes/scheme-buttons.ts` (роль → свойство → переменная), печатается в tokens.css;
  порты пяти тем помечают кнопку `data-scheme-button` и не красят её классами. Сторож
  `scheme-button-roles.spec.ts`.
- Данные тем (эталон снят с `<тема>.merfy.ru` 26.09, сторож `scheme-button-defaults.spec.ts`):
  - vanilla: новая схема 5 «олива + белый контур» (фон 58 69 48, кнопка = фон схемы / белый текст /
    белая рамка) для «Основного текста» и «Изображения с текстом» (сид главной + blockDefaults);
    «Подписка» по умолчанию — схема 2 (белая кнопка, оливковый текст); у схем 1, 2, 5 рамка
    дополнительной кнопки = её фон.
  - flux: новая схема 5 для «Товара» (сиды главной/товара + blockDefaults): «Добавить в корзину» —
    белая с синим контуром, «Купить сейчас» — синяя заливка.
  - satin: «Слайд-шоу» по умолчанию на схеме 4 — кнопка белая с чёрным текстом, читается на фото.
  - bloom: схема 2 — основная кнопка белая с чёрным текстом (как «Подробнее»/«К продукции» у
    верстальщиков); у схем 1–2 рамка по умолчанию = фон кнопки.
  - rose: без правок данных — вид уже совпадает.
- Отступления от вёрстки: вторая кнопка Hero (у верстальщиков её нет; прежний порт — прозрачный
  контур поверх фото) — заливка цветом схемы: rose — чёрная с белой рамкой, vanilla — светлая
  #eeeeee, flux — чёрная, satin — белая с серой рамкой, bloom — розовая. «Мультиряды —
  Дополнительная» у flux — чёрная заливка вместо контура.
- Существующие магазины (прод-БД 26.09, только чтение): у нового сайта `themeSettings` пустые —
  цвета из пакета темы; первая правка настроек темы в конструкторе сохраняет схемы в ревизию.
  Сохранённые схемы: bloom 15 (+3 палитры-заглушки конструктора), flux 22 (+1), satin 20 (+1),
  vanilla 25 (+2 тестовых стенда), rose 97 (+1 тестовый; у rose данные не менялись). До них
  доходят правило кнопок (вид «А») и то, что лежит вне сохранённых схем: новая схема 5 и
  `blockDefaults.colorScheme` для секций без своей схемы. Не доходят изменённые значения схем
  1–4 и сиды (у 26 vanilla-сайтов «Основной текст»/«Изображение с текстом» явно на схеме 2,
  у 18 flux-сайтов «Товар» на схеме 2). Миграция не делалась — решение за владельцем; PR #158
  не сливался. Палитры-заглушки (5 сайтов) — отдельной задачей.
- Владелец 26.09 выбрал вариант 2: слить и точечно перевести секции на схему 5 у сайтов, чья
  схема 2 совпадает со стандартной схемой темы (theme.json до PR, `158df1bb`; фон, заголовок,
  текст, фон/текст кнопок) или не сохранена. Отобрано 8 сайтов (vanilla 4 — «Основной текст» и
  «Изображение с текстом»; flux 4 — «Товар»), остальные 38 — сохранённые палитры (в основном
  старые сиды: vanilla дополнительная #3a4530, flux основная #fa5109) — не трогаем. Разовый скрипт
  после деплоя: новая ревизия + CAS по `current_revision_id` (как автосохранение), файл отката,
  витрины не публикуются. Скрипт и отчёт — вне репозитория (scratchpad задачи).
- Прочие сторожа: `vanilla-b41` (5 схем), `checkout-summary-scheme-surface` (23 связки),
  golden vanilla (сид схемы 5).

## 2026-09-26 — «после выбора темы схемы не применяются, нужен релоад» (все пять тем)

- Главная причина была в конструкторе (constructor PR #40, выкачен 26.09): после смены темы он
  сохранял свои запасные настройки (Comfortaa, скругление 8, схемы-заглушки) поверх `{}` пересева.
  Починены 11 затронутых магазинов (новые ревизии с CAS, откат лежит у агента в scratchpad).
- Вторая причина — в превью-агенте (`src/services/preview.service.ts`): `update-tokens`, пришедший
  до рукопожатия `init`, выбрасывался молча (правки секций в том же случае откладывались). Замер
  на проде, bloom: цвет схемы ушёл в iframe за 0,5 с до `init`, в базе сохранился, в превью
  появился только после перезагрузки. Теперь он откладывается так же, как `update-block`; из
  нескольких отложенных уходит последний.
- Гард: `src/services/__tests__/preview-agent-init-handshake.spec.ts` (+2 теста; на старом агенте
  2 красных, с правкой 8/8).
- Статус: владелец одобрил заливку превью-правки (26.09).

## 2026-09-26 — «Мультиколонны»: после выбора ссылки пропадала кнопка колонны (четыре темы)

- Владелец (26.09): «После выбора ссылки в мультиколонна пропала полностью кнопка в колоне».
- Воспроизведено на проде (тестовая учётка design-parity, черновик): «Мультиколонны» → «Колонна» →
  «Название ссылки» = «Подробнее» → «Ссылка» (страница «О нас», коллекция «Общая», свой адрес) —
  кнопка колонны пропадает сразу в превью, после перезагрузки конструктора и в серверном рендере
  `/preview?page=home` у rose, vanilla, flux, bloom. Вместе с кнопкой пропадает и заголовок
  колонны «Колонна». satin не задет: у его стартовых колонок нет легаси-поля `text`.
- Корень — общий код, не порты: `src/themes/page-blocks.ts`, `coerceLegacyValue`, правило
  `{text, link:{href}} → {text, href}` не проверяло состав ключей. Выбиралка ссылки пишет
  `link: {href, text?}`, у колонок стартового контента есть легаси `text` (описание), и вся колонка
  схлопывалась в `{text, href}` — терялись `heading`, `linkText`, `imageUrl`.
- Правка: конверт кнопки узнаётся только по составу ключей `BUTTON_ENVELOPE_FIELDS`
  (`text, link, enabled, href`) — тот же список, что уже был у соседнего правила со строковой
  ссылкой. Настоящие конверты кнопок (`rows[].button` у «Мультирядов» и т. п.) разворачиваются как
  раньше.
- Гард: `src/themes/__tests__/multicolumns-column-link-pick.spec.ts` — `adaptLegacyProps` + живая
  цепочка рантайма и порт каждой из пяти тем × три вида ссылки. На старом коде 18 из 19 красные,
  с правкой 19 из 19. Полный jest: те же 22 упавших набора, что на чистом origin/main (окружение),
  новых падений нет.
- Хвост (не чинилось): у rose/vanilla/flux/bloom подпись кнопки колонны берётся из подписи
  выбиралки (`link.text` = имя страницы/коллекции) раньше «Названия ссылки»; у satin — наоборот.
  После правки кнопка видна, но с текстом «О нас» вместо введённого «Подробнее» (кроме satin и
  своего адреса без подписи).

## 2026-09-26 — «Мультиколонны»: подпись кнопки колонки = «Название ссылки» (все пять тем)

- Владелец (26.09, «давай так да»): подпись кнопки колонки — «Название ссылки», пустое поле = кнопки
  нет (подсказка поля «Оставьте пустой, чтобы скрыть»), имя выбранной страницы/коллекции/товара
  подписью не становится. Делалось после слияния #158 (кнопки ↔ схемы), он трогал те же порты.
- Было: rose/vanilla/flux/bloom брали подпись из `link.text` выбиралки раньше «Названия ссылки»
  («О нас» вместо «Подробнее»); satin при пустом поле подставлял `link.text` и кнопку не скрывал.
- Сделано: общее правило `packages/theme-base/runtime/multicolumn-link.ts` (`multiColumnLink`),
  пять портов `themes/<t>/src/components/sections/MultiColumns.astro` зовут его вместо своих
  лестниц. Старый формат стартового контента `link: {text, href, enabled}` (узнаётся по `enabled`,
  выбиралка его не пишет) без поля «Название ссылки» рисуется как раньше. Заглушки пустого
  состояния перешли на пару `linkText` + `link` — «Кнопка» в каждой колонке осталась.
- Прод-БД (только чтение, текущие ревизии): колонок с выбранной ссылкой и пустым «Названием
  ссылки» — 0; объектные ссылки есть только в старом формате (22 колонки на 7 сайтах, rose/satin и
  без темы), с `enabled: "true"` и подписью «Ссылка» — у них кнопка остаётся.
...
## 2026-09-26 — Стёртый заголовок/текст секции остаётся пустым (все пять тем)

- Владелец (26.09, bloom «Коллекция товаров»): стёр заголовок и текст — на их месте снова текст
  темы; «должны чиститься просто и все». Решение владельца по неоднозначному случаю — «пусто
  везде», в том числе там, где дефолт поля был пустым, а секция показывала текст темы.
- Причина, три слоя: (1) `deepMergeBlockProps` (preview.service) подставлял `blockDefaults` темы
  вместо "" — правило 07.09 для `logo: ""`; (2) порты секций писали `x?.trim() || "Заглушка"` /
  `|| легаси title`; (3) `page-blocks` (ImageWithText/MainText) превращал «поля нет» в "", и
  порт не отличал «стёрто» от «не задано». Плюс `resolveBlockProps` делал «стёртым» конверт
  `heading: {size}` без текста.
- Правило (SECTION-CONTRACT §2–3, переписаны): строка из панели — рисуем как есть, пустая —
  элемента и его обёртки нет; поля нет вовсе — заглушка темы. Общий помощник
  `packages/theme-base/runtime/merchant-text.ts` (`textOf`/`pickText`/`hasText`), порты пяти тем
  переведены на него. Текст новой секции — в значении поля: `blockDefaults` «Галерея» (vanilla,
  satin), «Видео» (satin, flux). Кнопки не трогали.
- Намеренные изменения вида: «Основной текст» в старых ревизиях без полей показывает текст,
  который стоит в панели по умолчанию (раньше rose/flux — пусто); flux «Товар» без выбранного
  товара — без строки «Новая коллекция!». У неисправленных до этого секций с пустым дефолтом
  («Галерея» vanilla/satin, «Видео» satin/flux) нетронутый заголовок пропадёт — решение владельца.
- Сторожа: `src/themes/__tests__/cleared-text-stays-empty.spec.ts` (проба
  `cleared-text-probe.mjs`: стирает текстовые поля каждой секции пяти тем, сравнивает с рендером
  «метка» — ищет подставленный текст и пустые элементы; человекочитаемо —
  `node scripts/qa/probe-cleared-text.mjs <темы>`), `merchant-text.spec.ts`,
  `deep-merge-cleared-content.spec.ts`, `resolve-props-cleared-heading.spec.ts`; проверка §2 в
  `scripts/section-contract-check.mjs` ловит `|| "Заглушка"`. До правки проба: 36 секций с
  подставленным текстом; после первого круга — 0, но 31 с пустыми элементами; после второго — 0
...
## 2026-09-26 — «Спасибо за заказ»: цветовые схемы, новый дизайн, обычная шапка (общий блок, начато с bloom)

- Владелец (26.09, скриншот bloom): на странице «Спасибо» не работают цветовые схемы, дизайн сломан.
  Порядок работы: сначала bloom, перед заливкой — скриншоты всех схем на временной странице;
  владелец одобрил («да»). Шапка — обычная шапка магазина, шрифты и цвета — из настроек темы и
  параметров секции («как обычно»).
- Причина: `OrderConfirmation` не красил свой фон. Текст схемы (у bloom схемы 1–2 розовые с белым
  текстом) ложился на белую страницу — правая колонка пропадала; левую колонку заливало `--color-bg`
  без отступов; подложка сводки — `--color-input-bg` (белый во всех схемах bloom). Без схемы блок брал
  схему магазина по умолчанию (у bloom розовая Схема 1). Досев страницы старым сайтам ставил
  платформенную константу `scheme-2` (у bloom — розовая).
- Сделано (общий блок `packages/theme-base/blocks/OrderConfirmation`): секция красит фон схемой;
  без схемы — `.color-scheme-checkout` (светлая, как оформление заказа); вторичный текст, рамки и
  подложка сводки — цветом текста с прозрачностью (читаются в любой схеме); значок и кнопка — цвета
  кнопки схемы (в светлой платформенной — цвет заголовка). Новая вёрстка: номер заказа + приветствие,
  статус оплаты, детали сеткой (телефон и почта строками), сводка «Товары / Скидка / Доставка /
  Итого». Свои цвета мерчанта (`orderBg`, `summaryBg`, акцент, кнопки, ошибка) работают как раньше.
- Шапка: у «Спасибо» в ревизии лежала «Шапка оформления» без подвала — конструктор расходился с
  витриной (та рисует шапку магазина, `chrome: 'full'`). Стартовые страницы rose/bloom/satin/flux —
  промо + шапка + секция + подвал, как «О нас»; существующим ревизиям `storeChromeOnCheckoutResult`
  (revision-migrations) заменяет «Шапку оформления» группой шапки главной и добавляет подвал главной;
  досев страницы — без схемы. Тест: `revision-migrations-checkout-result-chrome.test.ts`.
- Проверка: стенд `renderPreviewPage` (тот же код, что превью) × без схемы + 4 схемы bloom, 1280 и
  390 px; `pnpm checks` — 8919 проверок, 0 упавших (инвентарь satin пересчитан после коммита).
- Хвост: у vanilla страницы «Спасибо» в теме нет. Остальные темы получают тот же блок — отдельно не
  сверялись скриншотами.

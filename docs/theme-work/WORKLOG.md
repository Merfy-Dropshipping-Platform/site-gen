...
## 2026-09-22 — поиск из лупы: выкатка и точность

- Владелец на обзоре: по «ра» находятся товары без «ра» в названии. Замер
  боевого API: сервис искал и по описанию — 26 результатов на девяти
  двухбуквенных запросах, 24 совпали только описанием.
- Починка в бэкенде: витрина передаёт `searchFields: ["title", "sku"]`
  (product #16, api-gateway #20), админка ищет как раньше, `%` и `_`
  экранируются, фасеты каталога считаются по тем же полям. Слито и выкачено;
  замер после: совпадений по описанию 0, `%%` → 0 вместо всего каталога.
- site-gen: поезд залил `608815be`. Его шаг CI нашёл прогон PR с той же головой
  (выкатка там пропущена) и опубликовал стенды до выкатки sites. Стенды
  переопубликованы вручную после `finished` в Coolify — у всех шести
  `sitesCommit=608815be`.
- Живой Playwright: rose «кр» 4, bloom «помада» 3, vanilla «рамка» 4,
  satin «шарф» 2, сайт владельца «ра» → «ничего не нашлось», flux-стенд без
  товаров → пусто, превью конструктора — выдача есть.
- Хвосты: фото в демо-магазинах (satin — 404 в MinIO, bloom — фото нет,
  rose `eeezt2btnhp0` — picsum режется ORB), лого «Bloom Pilot» наезжает на
  лупу на 375 (было и до), поезд читает CI прогона PR.

## 2026-09-23 — первая ступень поиска: выкатка

- Три PR, ревью «на говнокод» пройдено в каждом (правила данными, без лестниц if, вложенность ≤ 2):
  product #17, api-gateway #21, site-gen #95. Выкачены по порядку: product → gateway → сайты
  (`d79ee71f`), стенды переопубликованы.
- CI product поймал: миграции в CI запускает `npm run migration:run`, там режим транзакции был
  прежним. Флаг `--transaction each` добавлен в скрипты, сторож проверяет все четыре способа запуска.
- Индекс триграмм сначала не использовался: условие «текст ИЛИ вариант ИЛИ артикул». Переписано на
  `id IN (UNION источников)` — на магазине 20 000 товаров 5,5 → 1,1 мс, индекс в плане.
- Замер прода до/после и Playwright подсветки — в STATUS. Хвост: варианты сканируются по всей
  платформе, при росте каталога сузить до магазина.

## 2026-09-23 — «Найти» с одной буквы: шлюз и выкатка

Жалоба владельца: «кнопка найти должна ввести в каталог с теми товарами, которые нашлись».
После `minChars` 1 и починки превью (запись выше) нашлась третья поломка — в шлюзе.

- api-gateway #22 (`cca10b41`): `getProducts` и `getFilters` выбрасывали `q` короче двух символов
  (`query.q.length >= 2`) — подсказки по одной букве находили своё, а каталог после «Найти»
  отдавал всё. Теперь `catalogSearchQuery` (`src/store/catalog-search.ts`) только обрезает
  пробелы; из одних пробелов — без фильтра. Тесты `jest src/store` 198/198, из 8 новых на старом
  контроллере красные 6.
- Выкачено всё: site-gen #101 `1fe646cb` (CI и deploy-to-coolify зелёные), constructor #27
  `1bc880a5` (`customize.merfy.ru/build.json` → builtAt 01:31:30 UTC), api-gateway #22 `cca10b41`
  (build-and-test и deploy-to-coolify зелёные). Шесть витрин пересобраны из `1fe646cb`.
- Замер до/после на API, которым пользуются обе стороны (одна буква; «подсказки» — все страницы
  `products/search`, «каталог» — `store/products?q=` total):

  | тема | буква | подсказки | каталог до | каталог после |
  |---|---|---|---|---|
  | vanilla | к | 21 | 60 | 21 |
  | bloom | м | 3 | 6 | 3 |
  | satin | ш | 2 | 6 | 2 |
  ...
  По целому слову («кашпо», «маска», «шерстяной», «bloom») совпадало и раньше.
- Сквозной Playwright по пяти темам: превью — одна буква → подсказки → «Найти» → `navigate`
  `/__theme/<тема>/catalog?q=<буква>` → каталог по src конструктора = набор подсказок, чужих 0;
  flux — превью сайта `10caf133` (63 товара), «р» 1/1. Живые витрины vanilla/bloom/satin/rose —
  то же, адрес после «Найти» `/catalog?q=<буква>`. Настоящий конструктор (демо-учётка, сайт
  `71f9b323`): «в» → src `preview?page=page-catalog&q=в`, адрес `page=page-catalog`, каталог 1/1.
- Живой flux с товарами проверить нечем: у всех 11 опубликованных витрин flux на проде товаров
  нет. На стенде `u9fpo33bkmsd` с первой буквы уходит запрос и показывается «По запросу «р» ничего
  не нашлось», «Найти» ведёт на `/catalog?q=р`.
- Попутно: при выкатке шлюза rose и flux в прогоне поймали 502 («no available server») —
  контейнер шлюза меняется не бесшовно, вся витринная API на время подмены лежит.

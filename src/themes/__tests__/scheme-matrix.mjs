...
export const ALLOWED = [
  {
    ...
    why: 'плашка-заглушка под миниатюрой товара (серый квадрат) — эталон rose её не токенизирует, это подложка изображения, а не поверхность секции',
    test: (c) => /^(bg-\[#(F5F5F5|f5f5f5|FBFBFB|fbfbfb|EEEEEE|eeeeee)\])$/.test(c.cls) && /(^|\s)(size-|w-\d|h-\d|aspect-)/.test(c.nodeClasses),
  },
  {
    id: 'form-status',
  ...
  {
    id: 'media-scrim',
    why: 'полупрозрачная вуаль поверх фотографии (bg-black/NN) — затемнение медиа ради читабельности текста, а не поверхность',
    test: (c) => /^bg-black\/\d+$/.test(c.cls),
  },
];

...
export function resolveBlockProps(
  ...
  for (const [key, value] of Object.entries(props)) {
    ...
    if (role === "content") {
      ...
      // «Не задано» = пусто ЛИБО ровно дефолт блока/темы. Словарь служебных слов
      // (EMPTY_STATE_COPY) здесь больше не применяется: он съедал нормальный
      // мерчантский ввод — «Видео», «Коллекция», «Кнопка» превращались в заглушку.
      // «Не задано» = ПУСТО. Раньше сюда добавлялись словарь служебных слов и
      // сравнение с дефолтом блока — из-за них панель и превью показывали
      // разное: в поле стоял текст, а на экране заглушка (или наоборот).
      // Теперь правило одно и предсказуемое: что лежит в поле, то и рендерится;
      // пустое поле отдаётся блоку как пустое, и он рисует свою заглушку.

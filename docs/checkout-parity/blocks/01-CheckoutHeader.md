...
> **2026-09-13 — шапка больше не полоса во всю ширину.** По эталону владельца
> (Shopify-подобный чекаут) логотип стоит ВНУТРИ левой колонки: правая колонка
> идёт поверхностью от верха окна до низа, белой полосы над ней нет. Замер «до»
> (собранные витрины, Chromium, 1440×900): шапка 0..134.39 у rose и 0..132 у
> vanilla/flux/satin/bloom, обе колонки начинались с этой отметки; после —
> обе колонки от 0, логотип в левой колонке (x=298 при 1440, x=16 при 390).
> Разметка и стили — `CheckoutLayout/CheckoutSplit.astro` (слот `header`) и
> `CheckoutLayout/checkout-split.ts`. Гард: `src/themes/__tests__/checkout-header-in-column.spec.ts`.
> Следствие: контрол «Цветовая схема» у этой секции на чекауте больше ничего не
> меняет — палитру логотипа задаёт колонка. Из панели контрол НЕ убран.

...
const ПУСТЫЕ = аргументы.includes("--empty");
...
for (const тема of темыДляПрогона) {
  ...
  for (const секция of секцииТемы(тема)) {
    ...
    // --empty: секция ТОЛЬКО ЧТО ДОБАВЛЕНА (без своих фото и текста) — в таком
    // виде её и настраивает мерчант. Тестер 24.09: «Изображение по всем
    // настройкам не принимает изменения» — на пустой секции настройки молчали.
    const содержимое = ПУСТЫЕ ? {} : наполнение(секция, объявлены);

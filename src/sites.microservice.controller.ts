...
export class SitesMicroserviceController {
  ...
  async createSite(@Payload() data: any, @Ctx() _ctx: RmqContext) {
    try {
      ...
      if (!tenantId || !actorUserId || !name) {
        return {
          success: false,
          message: "tenantId, actorUserId and name are required",
        };
      }
  ...
  async listSites(@Payload() data: any) {
    try {
      ...
      this.logger.log(
        `Returning sites.list result: ${result.items?.length ?? 0} items`,
      );
  ...
  async deleteSite(@Payload() data: any) {
    try {
      ...
      if (!tenantId || !siteId)
        return { success: false, message: "tenantId and siteId required" };
      const ok = hard
        ? await this.service.hardDelete(tenantId, siteId)
        : await this.service.softDelete(tenantId, siteId);
      return { success: ok };
    } catch (e: any) {
  ...
  async attachDomain(@Payload() data: any) {
    try {
      ...
      if (!tenantId || !siteId || !domain || !actorUserId)
        return {
          success: false,
          message: "tenantId, siteId, domain, actorUserId required",
        };
      const res = await this.service.attachDomain({
        tenantId,
        siteId,
        domain,
        actorUserId,
  ...
  async createRevision(@Payload() data: any) {
    try {
      ...
      if (!tenantId || !siteId)
        return { success: false, message: "tenantId and siteId required" };
      const res = await this.service.createRevision({
        tenantId,
        siteId,
        data: revData,
        meta,

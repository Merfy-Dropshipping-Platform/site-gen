...
export const FooterPuckConfig = {
  ...
  fields: {
    ...
    // секцию, чтобы можно было там менять логотип»). `image` — общий загрузчик
    // конструктора (рамка, «Добавить фото», превью, «Изменить»); `maxSizeKb` —
    // тот же предел 500 КБ, что у логотипа шапки. Рендер — порты подвала через
    // packages/theme-base/runtime/footer-logo.ts. Та же строка стоит в satin
    // (свой конфиг подвала); совпадение сторожат satin-sidebar-canon.spec и
    // footer-logo.spec.
    ...
    // Скрыто по решению владельца: в панели подвала остаются только «Рассылка»
    // (заголовок/текст/размеры) и оформление (цветовая схема, отступы).
    // Колонки, соцсети и копирайт наполняются автоматически из настроек
    // магазина (политики, контакты), править их руками не нужно.

...
declare global {
  interface Window {
    __fluxAuth?: typeof fluxAuth;
    __fluxAuthBound?: boolean;
  }
}

export const fluxAuth = {
  // сторы
  $token,
  $customer,
  $authStatus,
...
/**
 * initAuthUI — выставляет window.__fluxAuth (для inline-скриптов карточек/хедера),
...
export function initAuthUI(): void {
  ...
  if (!window.__fluxAuthBound) {
    window.__fluxAuthBound = true;

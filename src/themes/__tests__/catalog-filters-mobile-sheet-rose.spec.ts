 ...
 *
 * 23.09 владелец: «адаптив мобилы взять с вёрстки». Верстальщики 17.09
 * переделали шторку во всех пяти темах (RoseFiltersSheet): это обёртка вокруг
 * ТОЙ ЖЕ строки выпадашек, что на десктопе. Ниже lg она — шторка под шапкой, от
 * lg — display:contents, и строка стоит на своём месте. Поэтому строка одна, и
 * живёт она внутри шторки. Поведение шторки (открыть, закрыть, Esc, замок
 * прокрутки) для пяти тем сторожит catalog-filters-sheet.spec.ts; здесь —
 * своё у rose: счётчик «(N)» на кнопке.
 *
 ...
 *   1) строка details-дропдаунов одна и лежит внутри шторки, которая от lg
 *      встаёт на место (lg:contents!) — регресс «строка снова развернулась на
 *      мобиле» = строку вынули из шторки;
 ...
 *      внутри него;
 *   3) шторка (`role="dialog"`) скрыта по умолчанию классом `hidden`, несёт
 *      «Показать»/«Закрыть»;
...
describe("rose /catalog: мобильный адаптив фильтров и сортировки (эталон rose.merfy.ru)", () => {
  ...
  test("строка details-дропдаунов одна и лежит в шторке, которая от lg встаёт на место", () => {
    const bars = root.querySelectorAll('[data-nt="catalog-filters"]');
    expect(bars.length).toBe(1);
    const sheet = bars[0].closest("[data-filters-sheet]");
    expect(sheet).not.toBeNull();
    const cls = (sheet!.getAttribute("class") ?? "").split(/\s+/);
    expect(cls).toContain("lg:contents!");
  });
  ...
  test("шторка существует, скрыта по умолчанию и несёт «Показать»/«Закрыть»", () => {
    const sheet = root.querySelector("[data-filters-sheet]");
    expect(sheet).not.toBeNull();

...
describe("матрица «тема × секция × мишень» против цветовой схемы", () => {
  ...
  it("белый список: каждое исключение объяснено", () => {
    ...
    expect(guard.allowed.map((a) => a.id).sort()).toEqual([
      ...
      "hero-overlay-veil",
      // Заведено 18.09 при разборе жалобы владельца п.5 («не применяется цв
      // схема к кнопке при наведении»): подложка КАДРА товара — не кнопка,
      // сканер принимает её за кнопку из-за aria-label на контейнере. См.
      // объяснение в ALLOWED.
      "product-media-tile-hover",

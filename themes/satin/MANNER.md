...
| Поиск | коробка выпадает поверх страницы под шапкой (макет «Поиск» 1320×374): строка 48 + чёрная «Найти», ниже живая выдача — горизонтальный ряд карточек 195×256 на «Поверхности» схемы (плашка «Скидка», сердце избранного). Данные — gateway `/api/store/products/search` через общий модуль `theme-base/runtime/header-search`, вид — `src/lib/header-search-view.ts`; демо-индекс (`data-search-index`) снят 22.09. На телефоне лупа открывает поиск в шторке (сетка 2 колонки) | Header.astro, lib/header-search-view.ts |
| Бургер | fixed-панель под фактическим низом шапки (`top-[56px]` — фолбэк): строка поиска с «Найти» + чёрный квадрат профиля h-10 w-10 (иконка цветом «Текст кнопки»); пока показана выдача поиска, меню спрятано; навигация колонкой gap-4 text-xs uppercase, активная плашка `h-0.5 w-12`; + пункты Корзина/Регистрация | H:172-198 |

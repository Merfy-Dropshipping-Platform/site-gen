...
export class BillingSyncScheduler {
  ...
  private rpc<T = any>(client: ClientProxy, pattern: string, data: any): Promise<T> {
  ...
  async reconcileBilling() {
    ...
    try {
      ...
      for (const tenantId of tenantIds) {
        try {
          const userRes: any = await this.rpc(this.userClient, 'user.get_tenant_billing_account', { tenantId });
          ...
          const entitlements: any = await this.rpc(this.billingClient, 'billing.get_entitlements', { accountId });

 ...
 */
/**
 * Живая правка «Цветовой схемы» на чекауте: обёртку вокруг секции НЕ создаём.
 *
 * У мега-блоков чекаута нет scheme-обёртки, а общая ветка агента создавала её
 * САМА — внутри колонки появлялся цветной прямоугольник под контентом, то
...
(function () {
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      // Подсветка наведения — ТОЛЬКО рамка. Полупрозрачная заливка поверх секции
      // читалась как «свечение» на самом контенте (пользователь: «убрать свечение»)
    ...
    if (ev.data.type === 'set-selection') {
      ...
      fetch('/api/sites/' + currentSiteId + '/preview/block', {
        ...
        .then(function (payload) {
          ...
          if (!el) {
            // b85-fix: id конструктора разошёлся с id в разметке (причины см.
            // ниже). Раньше правка просто терялась: владелец переключал
            // «Цветовую схему» у секции «Страница», конструктор слал
            // update-block с id Page-1, а в живом DOM лежал Page-<uid> —
            // сервер отдавал ПРАВИЛЬНЫЙ фрагмент (проверено curl'ом: scheme-3
            // в ответе), но применять его было некуда. Выглядело как «схема не
            // применяется», хотя после перезагрузки страница красилась верно.
            //
            // Чиним без догадок о причине рассинхрона: если на странице ровно
            // ОДИН блок этого типа, двусмысленности нет — правка адресована
            // ему. Замена outerHTML заодно приводит id в разметке к тому,
            // который знает конструктор, поэтому следующая правка попадёт уже
            // по прямому селектору. Два и больше блоков одного типа — не
            // угадываем, оставляем прежнее поведение с ошибкой в консоли.
            var sameType = document.querySelectorAll('[data-puck-component-id^="' + blockType + '-"]');
            if (sameType.length === 1) {
              el = sameType[0];
              console.warn('[preview] update-block: blockId=' + blockId + ' не найден, но блок типа '
                + blockType + ' на странице один — применяем правку к нему');
            }
          }
          if (!el) {
          ...
          // Чекаут: обёртку вокруг секции не создаём — иначе внутри колонки
          // красится пятно под контентом, и превью расходится с витриной

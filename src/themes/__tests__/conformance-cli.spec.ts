...
function req(id: string, overrides: Partial<RequirementRecord> = {}): RequirementRecord {
  return {
    id,
...
function makePipeline(over: Partial<ConformancePipelineResult> = {}): ConformancePipelineResult {
  return {
    theme: 'bloom',
...
function harness(): Harness {
  ...
  return {
    dir,
    reviewedRequirementsPath: join(dir, 'conformance', 'requirements', 'bloom.v1.json'),
    baselinePath: join(dir, 'conformance', 'baselines', 'bloom.structural.json'),
    inventoryPath: join(dir, 'conformance', 'inventory', 'bloom.generated.json'),
    reportDir: join(dir, 'conformance-results', 'bloom'),
  };
}

function writeReviewed(h: Harness, reqs: RequirementRecord[]): void {
  writeFileSync(h.reviewedRequirementsPath, JSON.stringify(reqs, null, 2), 'utf8');
}
  ...
  return {
    ...
    checkPrerequisites: async () =>
      opts.prerequisitesOk === false
        ? { ok: false, missing: ['dist/src/controllers/theme-puck-config.controller.js'] }
        : { ok: true, missing: [] },
...
function normalArgs(h: Harness): string[] {
  return [
    ...
    '--inventory', h.inventoryPath,
    '--report-dir', h.reportDir,
    '--requirements', h.reviewedRequirementsPath,
  ];
}
...
function seedBaseline(
  ...
  return { inventoryDigest: cand.inventoryDigest, reviewDigest: cand.reviewDigest };
}
...
describe('--propose-requirements', () => {
  it('writes a deterministic local proposal without reading/mutating reviewed artifacts', async () => {
    ...
    const res = await runThemeConformance(
      ['--theme', 'bloom', '--report-dir', h.reportDir, '--propose-requirements', proposalPath],
      makeDeps(h, pipeline),
    );
...
describe('--diagnose', () => {
  it('permits missing baseline/inventory, writes report, exits 0, mutates nothing', async () => {
    const h = harness();
    writeReviewed(h, [req('bloom.block.Hero.title')]);
    const res = await runThemeConformance(diagnoseArgs(h), makeDeps(h, makePipeline()));
    expect(res.ok).toBe(true);
    expect(res.exitCode).toBe(0);
    expect(existsSync(h.baselinePath)).toBe(false);
    expect(existsSync(h.inventoryPath)).toBe(false);
  ...
  it('two unchanged diagnose runs produce byte-identical report + stable review digest', async () => {
    const h = harness();
    writeReviewed(h, [req('bloom.block.Hero.title')]);
    const r1 = await runThemeConformance(diagnoseArgs(h), makeDeps(h, makePipeline()));
    const json1 = readFileSync(join(h.reportDir, 'report.json'), 'utf8');
    const md1 = readFileSync(join(h.reportDir, 'report.md'), 'utf8');
    const r2 = await runThemeConformance(diagnoseArgs(h), makeDeps(h, makePipeline()));
...
describe('normal mode', () => {
  it('passes when the tracked inventory + baseline exactly match', async () => {
    ...
    const res = await runThemeConformance(normalArgs(h), makeDeps(h, makePipeline()));
    expect(res.ok).toBe(true);
    expect(res.exitCode).toBe(0);
  });
  ...
  it('fails on a stale (byte-mismatched) tracked inventory', async () => {
    ...
    writeFileSync(h.inventoryPath, readFileSync(h.inventoryPath, 'utf8') + ' ', 'utf8');
    const res = await runThemeConformance(normalArgs(h), makeDeps(h, makePipeline()));
    expect(res.ok).toBe(false);
  ...
  it('fails on baseline/inventory digest mismatch', async () => {
    ...
    const b = JSON.parse(readFileSync(h.baselinePath, 'utf8')) as StructuralBaseline;
  ...
  it('fails with an unexpected finding (new UNKNOWN capability expansion)', async () => {
    ...
    const reviewed = [req('bloom.block.Hero.title'), req('bloom.block.Hero.subtitle')];
    writeReviewed(h, reviewed);
    const expanded = makePipeline({
      capabilities: [cap('bloom.block.Hero.title'), cap('bloom.block.Hero.subtitle')],
    });
  ...
  it('fails on a changed-fingerprint baseline', async () => {
    ...
    b.findings = b.findings.map((f, i) =>
      i === 0 ? { id: f.id, fingerprint: sha('flipped') as `sha256:${string}` } : f,
    );
    writeFileSync(h.baselinePath, JSON.stringify(b, null, 2), 'utf8');
    const res = await runThemeConformance(normalArgs(h), makeDeps(h, makePipeline()));
    expect(res.ok).toBe(false);
  ...
  it('fails on a missing locked requirement (code-field deletion)', async () => {
    ...
    const reviewed = [req('bloom.block.Hero.title'), req('bloom.block.Hero.deleted')];
    writeReviewed(h, reviewed);
...
describe('--require-zero', () => {
  it('fails while any finding exists even if it matches the baseline', async () => {
    ...
    writeReviewed(h, reviewed);
    seedBaseline(h, makePipeline(), reviewed);
    const res = await runThemeConformance(releaseArgs(h), makeDeps(h, makePipeline()));
    expect(res.ok).toBe(false);
    expect(res.exitCode).not.toBe(0);
    // no mutation
  ...
  it('passes only when there are zero findings', async () => {
    ...
    const pipeline = makePipeline({ capabilities: [cap('bloom.block.Hero.title', { status: 'PASS', editable: false })] });
    const reviewed: RequirementRecord[] = [];
    writeReviewed(h, reviewed);
    seedBaseline(h, pipeline, reviewed);
    const res = await runThemeConformance(releaseArgs(h), makeDeps(h, pipeline));
    expect(res.ok).toBe(true);
    expect(res.exitCode).toBe(0);
  });
});
...
describe('--capture-initial-baseline', () => {
  ...
  it('captures a fresh baseline + inventory when everything is valid', async () => {
    ...
    expect(readFileSync(h.inventoryPath).equals(cand.inventoryBytes)).toBe(true);
    const stored = JSON.parse(readFileSync(h.baselinePath, 'utf8')) as StructuralBaseline;
  ...
  it('refuses with a bad acknowledgement', async () => {
    ...
    const res = await runThemeConformance(
      captureArgs(h, cand.reviewDigest),
      makeDeps(h, makePipeline(), {
        env: { BLOOM_BASELINE_ACK: 'wrong', BLOOM_INVENTORY_ACK: 'reviewed-refresh' },
      }),
    );
    expect(res.ok).toBe(false);
    expect(existsSync(h.baselinePath)).toBe(false);
  ...
  it('rejects a review-digest that no longer matches current sources (zero writes)', async () => {
    ...
    const oldCand = expectedCandidate(makePipeline({ sourceDigest: sha('OLD') }), reviewed);
    const res = await runThemeConformance(
...
describe('--shrink-baseline', () => {
  ...
  it('refuses to shrink when the current findings ADD a finding', async () => {
    ...
    const grown = makePipeline({
      capabilities: [cap('bloom.block.Hero.title'), cap('bloom.block.Hero.added')],
    });
  ...
  it('shrinks when the current findings are a strict subset (at least one removed)', async () => {
    ...
    const big = makePipeline({
      capabilities: [cap('bloom.block.Hero.title'), cap('bloom.block.Hero.extra')],
    });
...
describe('--append-requirement-locks', () => {
  ...
  it('appends a new requirement lock while the finding set stays byte-identical', async () => {
    ...
    const reviewedNew = [req('bloom.block.Hero.title'), req('bloom.block.Hero.also')];
    ...
    expect(res.ok).toBe(true);
    const stored = JSON.parse(readFileSync(h.baselinePath, 'utf8')) as StructuralBaseline;
    expect(stored.requirements.some((r) => r.id === 'bloom.block.Hero.also')).toBe(true);
  });

  it('refuses append on a no-op (no new lock)', async () => {
    const h = harness();
    const pipeline = makePipeline({
      capabilities: [cap('bloom.block.Hero.title', { status: 'PASS', editable: false })],
    });
    const reviewed = [req('bloom.block.Hero.title')];
    writeReviewed(h, reviewed);

...
): number | string | null | undefined => v;
...
export function findCollection(
  ...
  return (
    collections.find(
      (c) => c && (c.id === ref || c.slug === ref || c.name === ref),
    ) ?? null
  );
}
...
export function filterByCollection(
  ...
  if (filtered.length === 0) {
    filtered = products.filter((p) => {
      const memb = (
        p as unknown as { collections?: Array<{ id?: string; slug?: string }> }
      ).collections;
      return (
        Array.isArray(memb) &&
        memb.some(
...
function wishlistHeartHtml(id: string): string {
  const w =
    typeof window !== "undefined"
      ? (window as unknown as { __fluxWishlist?: WishlistGlobal })
          .__fluxWishlist
      : undefined;
  const fav = !!w?.has?.(id);
...
export async function hydrateGrid(
  ...
  const grids = Array.from(
    document.querySelectorAll<HTMLElement>(gridSelector),
  );
  ...
  const html = items
    .map(
      (p) =>
        `<li data-product-id="${escapeHtml(p.id)}">${renderCardHtml(p)}</li>`,
    )
    .join("");
...
export function updateCount(countSelector: string, n: number): void {
  for (const el of Array.from(
    document.querySelectorAll<HTMLElement>(countSelector),
  )) {
  ...
  return (
    combos.find((c) =>
      Object.entries(selected).every(
        ([group, value]) => c.options?.[group] === value,
      ),
    ) ?? null
  );
}
 ...
 */
function renderVariantGroupWrapper(
  groupName: string,
  controlHtml: string,
): string {

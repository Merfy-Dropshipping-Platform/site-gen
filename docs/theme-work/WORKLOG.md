...
## 2026-09-10 — баг-репорт владельца (40 пунктов): что закрыто и что ждёт решения

Закрыто (19): корзина satin в гейте унификации; легаси CheckoutLayout в ревизии flux; сид корзины rose; Ctrl+Z; подтверждение удаления; контакты магазина; адреса разделов настроек; новая страница (шапка + пункт меню); двойной копирайт; компактная заглушка «Видео»; размер заголовка «Контактной формы» rose; текст кнопки баннера; вкладка «Кнопка» у «Основного текста»; жаргон в чекауте; отступы шапки bloom/satin; плюс ранее — наведение на кнопки, обводка/скругление/отступ карточки, выравнивание, тексты корзины.

Не воспроизвелись (6): выравнивание карточки, клик по шкале ползунка, возврат скрытой секции, вкладки настроек, теги <em> как текст, лишние запросы/401.

Ждут решения владельца (2): единый ритм отступов секций (у bloom в сиде 120/80/0 — эталона нет) и дефолт «Регистр текста» промо-баннера (bloom/satin сеют uppercase — это выбор верстальщиков).

Технические уроки дня: правка порта секции требует compile-theme-sections И перезапуска sites (модуль остаётся в ESM-реестре); scoped-<style> секции в CSS темы не попадает — только global.css; замерять размеры нужно в контексте темы, иначе получаешь дефолт браузера; переносить правки между воркtree только точечно, не копированием файла.

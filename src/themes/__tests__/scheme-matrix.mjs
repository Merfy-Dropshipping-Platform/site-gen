...
function parseCompound(compound) {
  ...
  s = s.replace(/::?[a-z-]+(\([^)]*\))?/gi, '');
  // Имя класса/id — чередование ЭКРАНИРОВАННЫХ пар (`\X` — литерал X, ровно
  // так CSS-парсер разбирает escape-последовательность) и обычных символов
  // идентификатора. НЕЭКРАНИРОВАННЫЙ `[` (и `]`, `#`) эту последовательность
  // ЗАКАНЧИВАЕТ — это начало настоящего атрибутного селектора (`a[href]`)
  // или нового id (`.foo#bar`), а не часть имени Tailwind-класса.
  //
  // БЫЛО: `[`/`]` лежали в общем наборе символов имени НАПРЯМУЮ (без разбора
  // экранирования), а `#` там не было вовсе. Поэтому жёстко экранированный
  // Tailwind-класс `.bg-\[\#000000\]` резался НАДВОЕ прямо на непарном `#`:
  // `bg-\[\` дочитывался как имя класса (обрыв на первом символе вне
  // старого набора), а `#000000\]` — как СТАРТ НОВОГО id-селектора.
  // compoundMatches проверял `node.classes.includes('bg-[\\')` — такого
  // класса не бывает ни на одном узле, и правило `.bg-\[\#000000\]` не
  // матчилось НИКОГДА. Кнопка/текст с литеральным цветом просто не получали
  // это объявление кандидатом в `winningDecl` и выпадали из мишеней матрицы
  // целиком — не «краснели», а были НЕВИДИМЫ.
  const NAME = String.raw`(?:\\.|[A-Za-z0-9_@().,%/:!*+~='"^$&{}|<>?-])+`;
  const re = new RegExp(`([#.])(${NAME})|\\[([^\\]]+)\\]|^([a-zA-Z][a-zA-Z0-9-]*)`, 'g');
  let m;
  let first = true;
  while ((m = re.exec(s))) {
...
export const ALLOWED = [
  {
    id: 'product-media-tile-hover',
    why: 'подложка КАДРА товара (data-cfg-hero / data-media-index), а не кнопка: её фон — роль --color-surface, и виден он только пока грузится фото, поверх лежит сам снимок на всю плитку. Наведение здесь по дизайну приглушает плитку прозрачностью (hover:opacity-*), красить подложку заново незачем и нечему — сканер принимает узел за кнопку из-за aria-label на контейнере. Проверено 18.09 при разборе жалобы владельца п.5: у кнопок «Слайд-шоу» (и всех прочих) наведение чинилось по-настоящему, эта клетка — единственная, где мишень не кнопка',
    test: (c) =>
      c.kind === 'button-hover' &&
      c.prop === 'background-color' &&
      c.chain.includes('--color-surface') &&
      Object.keys(c.node.attrs).some((k) => /^data-(cfg-hero|media-index)$/.test(k)),
  },
  ...
  {
    id: 'counter-badge',
    why: 'ПЛАШКА (фон) счётчика корзины и избранного в шапке — фирменный цвет темы, не роль схемы: rose/bloom/flux/satin пишут его ЛИТЕРАЛОМ (`bg-[#000000]` у rose/satin, `bg-[#e38e9f]` у bloom, `bg-[#1e2952]` у flux — вердикт `hex`), у vanilla пара зеркальная — белая плашка с тёмной надписью на алиасе --vanilla-dark (вердикт `theme-var`/`palette`, тема не пишет её литералом). Перекрасить фон значит поменять фирменный узнаваемый вид плашки — не дефект. СУЖЕНО ДО ФОНА 16.09 (было — и фон, и цифра): цифра — это отдельная роль (--color-button-text), она читаема на плашке ЛЮБОГО цвета и обязана следовать схеме сама по себе; белым списком больше не прикрыта — «плашка количества» из жалобы владельца теперь настоящая мишень матрицы. ДОБАВЛЕН `hex` 17.09 (b61): до починки разбора класса (`parseCompound`) селектор `.bg-\[\#e38e9f\]` не матчился НИ НА ОДНОМ узле — клетка была невидима матрице целиком, а не «palette», поэтому ветку `hex` тут никто ни разу не проверял на живой клетке. Как только клетка стала видна, выяснилось, что четыре из пяти тем несут её именно литералом, а правило её не прикрывало — ровно тот сценарий, о котором предупреждает комментарий выше словами «rose bg-rose-primary»: в роли уже нет такого класса, код разошёлся с комментарием, а тест остался в стороне от реальной клетки',
    test: (c) =>
      ['palette', 'hex'].includes(c.verdict) &&
      c.prop === 'background-color' &&
      ('data-cart-count' in c.node.attrs || 'data-wishlist-count' in c.node.attrs),
  },
  {
    id: 'satin-photo-canvas',
    why: 'satin несёт полотно ПОД full-bleed фото (или его дизайн-плейсхолдером — «Контракт картиночных секций, правило 2»: картинка показывается ВСЕГДА, светлая surface-rect не остаётся) фиксированным #111111, а не ролью схемы — паттерн повторён СОГЛАСОВАННО минимум в 6 файлах темы (Hero, ImageWithText, MultiRows, Slideshow, CollectionRows при row.image, ImageBanner) и явно противопоставлен «пустой surface» веткой того же тернарника (CollectionRows.astro:118: `row.image ? "bg-[#111111]" : "bg-[rgb(var(--color-surface,251_251_251))]"`) — тёмный холст ТОЛЬКО под фотографией, светлая роль схемы — когда фото нет вовсе. Другие картиночные секции satin (Journal, Publications, Gallery), где фото — не full-bleed, а карточка внутри рамки, используют --color-surface как и rose. НЕ УНИФИЦИРОВАНО с rose/bloom/flux/vanilla (там везде --color-surface даже под full-bleed фото) — это РАСХОЖДЕНИЕ с эталоном, оставленное белым списком, а не тихой правкой: смена тёмного холста на светлый — визуально заметная правка через 6+ файлов темы, которую эта задача (b61, починка разбора класса сканера) не уполномочена делать без владельца. Если владелец сочтёт унификацию нужной — снять эту запись и перевести все 6 мест на --color-surface, как здесь уже сделано для flux Popular.astro placeholder-карточки',
    test: (c) => c.theme === 'satin' && c.prop === 'background-color' && /^#111111$/i.test((c.value ?? '').trim()),
  },
  {

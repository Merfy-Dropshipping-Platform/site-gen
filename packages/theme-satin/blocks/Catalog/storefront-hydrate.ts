...
// дублируется намеренно (block-папка автономна). Фолбэк, пока __satinWishlist не
// готов — initWishlistUI перекрасит filled/aria-pressed на загрузке.
...
/** Overlay-сердце «Избранное» для карточки (СНАРУЖИ <a>, острые углы satin).
 *  filled берётся из глобала __satinWishlist (если готов) — иначе outline-фолбэк. */
function wishlistHeartBtnHtml(id: string): string {
	const w =
		typeof window !== "undefined"
			? (window as unknown as { __satinWishlist?: { has(id: string): boolean; heartSvg(f: boolean): string } }).__satinWishlist
			: undefined;
	const fav = !!(w && w.has(id));
	const inner = w ? w.heartSvg(fav) : WISHLIST_OUTLINE_SVG;
	return (
...
export function renderCardHtml(p: RealProduct, cardContainerOn = false): string {
	...
	const oldPrice = oldRaw
		? `<span class="font-manrope text-[14px] font-light leading-tight text-[rgb(var(--color-text,0_0_0)/0.75)] line-through">${escapeHtml(oldRaw)}</span>`
		: "";
	const badge = oldRaw

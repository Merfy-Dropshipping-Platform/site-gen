 ...
 * счёт: цена в строке, в сайдбаре «Сбоку» и в сайдбаре шторки — полей шесть.
 */
...
describe("«Стоимость» — поля ввода в обеих раскладках фильтра", () => {
  ...
  // Цена стоит в трёх местах: строка сверху, сайдбар «Сбоку» и сайдбар шторки
  // «Фильтры и сортировка» на телефоне (вёрстка vanilla, 23.09) — по два поля.
  it("vanilla: цена в строке, в сайдбаре и в шторке — полей шесть", async () => {
    await оживить("vanilla", "side");
    expect(document.querySelectorAll("[data-price-input]").length).toBe(6);
  });
});

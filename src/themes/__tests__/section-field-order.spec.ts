...
const CASES: Case[] = [
  ...
  },
  {
    ...
    block: "Hero",
    marker: HERO_MARKER,
    // Плейсхолдер = поля НЕ заданы вовсе (ключей heading/text/primaryButton
    // нет совсем), а не стёрты пустой строкой. 26.09: баг владельца «стёрли
    // текст — вместо пустоты показывает заглушку» чинится тем, что заглушка
    // при heading:{text:""}/text:{content:""} больше не рисуется (satin) —
    // явно очищенное поле теперь рендерится КАК ЕСТЬ (пусто → элемента нет),
    // и `drawnOrder` находит только "buttons". Порядок героя на самой
    // заглушке (эта проверка) держится на реально пустых пропсах.
    props: { ...base },
  },
  {
    name: "«Основной текст»",

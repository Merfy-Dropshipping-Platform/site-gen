 ...
 *      мерчанту нечем. Рядом, в этой же секции, старая цена (на 13.09) была
 *      сделана правильно: `rgb(var(--color-text))/60`.
 *
 * 19.09 (5e9201a4, докатилось до ProductPrice.astro партией batch3 25.09) —
 * отдельное решение владельца ИМЕННО про цену скидки: без приглушения вовсе
 * (не «/60», а `--color-text` целиком). Описание в этом решении не упомянуто
 * и не менялось — его приглушение осталось прежним; эталон T7 «старая цена
 * не съехала» ниже обновлён на новое значение (без /60), проверка самого
 * описания меряет его напрямую и не зависит от файла цены.
 *
...
describe.each(THEMES)("«Товар» / %s: цвета внутри схемы", (theme) => {
  ...
  it("T7 старая цена (после 19.09 — без приглушения; описание больше НЕ равняется на неё)", () => {
    // 13.09: старая цена была `--color-text)/60` и служила ОБРАЗЦОМ для
    // приглушённого описания (см. докстринг файла). 19.09 (5e9201a4, партия
    // batch3 25.09 донесла её до ProductPrice.astro) — отдельное, более узкое
    // решение владельца: ИМЕННО цена скидки красится ТЕКСТОМ БЕЗ приглушения
    // («цвет скидки должен быть как у текста», без слова «описание»). С этого
    // момента цена и описание — два РАЗНЫХ эталона; проверка описания выше
    // (T7 «описание приглушается...») меряет описание напрямую и от этой
    // правки не зависит.
    ...
    expect(priceOld).toContain("text-[rgb(var(--color-text))] line-through");
    expect(priceOld).not.toMatch(/--color-text\)\]\/\d/);
  });

 ...
 * «Высота» ожила.
 *
 * Откуда задача. 19.09 секционная «Высота» начала править высотой рядов. Фолбэк порта пришлось
 ...
 *
 * ПОПРАВКА 20.09. Правило, которым тогда оживили «Высоту», снято (оно
 * отменяло выбранный размер ряда — см. `multirows-row-size.spec.ts`). Секция
 * снова лишь фолбэк для ряда без своего размера, но миграция остаётся нужной:
 * она делает прежний молчаливый фолбэк явным значением, не меняя вида.
 *
 * Что сторожим:

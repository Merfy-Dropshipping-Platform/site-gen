...
export function declaredVar(
  ...
  for (const cls of [...classes].reverse()) {
    ...
    // Фон пишут и СОКРАЩЁННО: `.account-button`/`.auth-button-primary` объявлены
    // как `background: rgb(var(…))`, и без этой ветки проверка падала «ни один
    // класс не объявляет background-color» на мишени, которая на деле покрашена
    // (поймано 15.09 при расширении списка мишеней).
    ...
    // фолбэки; перебить инлайном предка нельзя НИ ОДНУ из них, иначе цвет
    // приедет из перебивки, а не из схемы.

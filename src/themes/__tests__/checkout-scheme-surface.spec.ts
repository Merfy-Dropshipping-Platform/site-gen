...
describe('общая split-колонка: поверхность по эталону', () => {
  ...
  it('колонка сводки красится токеном схемы, а не литералом (16.09: тем же --color-bg, что и форма)', () => {
    // 16.09: `--color-checkout-surface` (14.09/15.09) снят — владелец показал
    // скриншот, где обе колонки одной схемы визуально РАЗНЫЕ, и попросил
    // убрать различие целиком. Правило сводки теперь ПОБУКВЕННО совпадает с
    // правилом формы — читает `--color-bg`, а не отдельный токен поверхности.
    expect(css).toMatch(
      /\[data-checkout-pane="summary"\][^}]*background:\s*rgb\(var\(--color-bg,\s*255 255 255\)\)/,
    );
  });

...
export class DomainClient implements OnModuleInit {
   ...
   */
  private async generateSubdomainViaRpc(
    tenantId: string,
  ): Promise<GeneratedSubdomain | null> {
    if (!this.domainClient) {
      return null;
    }
    ...
    try {
      const response = await firstValueFrom(
        this.domainClient
          .send<
            RpcResponse<GeneratedSubdomain>
          >("domain.generate_subdomain", { tenantId })
          .pipe(
            timeout(this.rpcTimeout),
            catchError((err) => {
              this.logger.warn(
                `RPC domain.generate_subdomain error: ${err.message}`,
              );
              return of(null);
            }),
          ),
      ...
      this.logger.log(
        `RPC: Generated subdomain: ${domain.name} (id: ${domain.id}) for tenant ${tenantId}`,
      );
      ...
      this.logger.warn(
        `RPC generateSubdomain failed: ${error instanceof Error ? error.message : error}`,
      );
      return null;
    }
  }
   ...
   */
  private async generateSubdomainViaHttp(
    tenantId: string,
  ): Promise<GeneratedSubdomain> {
    const url = `${this.httpBaseUrl}/domains/generate-subdomain`;

    this.logger.log(
      `HTTP fallback: Generating subdomain for tenant ${tenantId} via ${url}`,
    );
    ...
    try {
      ...
      if (!res.ok) {
        ...
        this.logger.error(
          `Domain service HTTP error ${res.status}: ${errorText}`,
        );
      ...
      this.logger.log(
        `HTTP: Generated subdomain: ${data.name} (id: ${data.id}) for tenant ${tenantId}`,
      );
      ...
      this.logger.error(
        `HTTP generateSubdomain failed: ${e instanceof Error ? e.message : e}`,
      );
      ...
      try {
        const response = await firstValueFrom(
          this.domainClient
            .send<
              RpcResponse<GeneratedSubdomain>
            >("domain.create_custom_subdomain", { subdomain })
            .pipe(
              timeout(this.rpcTimeout),
              catchError((err) => {
                this.logger.warn(
                  `RPC domain.create_custom_subdomain error: ${err.message}`,
                );
                return of(null);
              }),
            ),
        ...
        if (response?.success && response.domain) {
          this.logger.log(
            `RPC: Created custom subdomain: ${response.domain.name}`,
          );
          return {
  ...
  async getDomain(domainName: string): Promise<GeneratedSubdomain | null> {
    ...
    try {
      const res = await fetch(
        `${this.httpBaseUrl}/domains/${encodeURIComponent(domainName)}`,
        {
          method: "GET",
      ...
      this.logger.warn(
        `HTTP getDomain failed: ${e instanceof Error ? e.message : e}`,
      );
      return null;
    }
  }
  ...
  async healthCheck(): Promise<boolean> {
    ...
    if (this.rpcAvailable && this.domainClient) {
      try {
        const response = await firstValueFrom(
          this.domainClient
            .send<{ success: boolean }>("domain.health", {})
            .pipe(
              timeout(3000),
              catchError(() => of(null)),
            ),

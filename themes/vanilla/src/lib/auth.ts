...
declare global {
  interface Window {
    __vanillaAuth?: typeof vanillaAuth;
    __vanillaAuthBound?: boolean;
  }
}

export const vanillaAuth = {
  // сторы
  $token,
  $customer,
  $authStatus,
...
/**
 * initAuthUI — выставляет window.__vanillaAuth (для inline-скриптов карточек/хедера),
...
export function initAuthUI(): void {
  ...
  window.__vanillaAuth = vanillaAuth;
  ...
  if (!window.__vanillaAuthBound) {
    window.__vanillaAuthBound = true;
    $isLoggedIn.subscribe((loggedIn) => paint(loggedIn));
    document.addEventListener('astro:page-load', () => paint(!!$token.get()));
    // vanilla: иконка-кнопка [data-login-open] открывала демо LoginDrawer.
    // Перехватываем в capture-фазе (опережая drawer-обработчик) → навигация на
    // страницу /login (гость) или /account (вошёл) — единый паттёрн с темами.

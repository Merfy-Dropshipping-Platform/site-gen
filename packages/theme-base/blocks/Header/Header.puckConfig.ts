...
export const HeaderSchema = z.object({
   ...
   * logo + actions; row 2 is centered nav menu. `rose-classic` (rose pilot)
   * — single row с pixel-perfect overrides: 1920px nav width, responsive
   * 2xl:px-[280px] / 2xl:gap-[80px], Comfortaa 20px logoText, увеличенный
   * navLink scale (xl:20px), shadow-sm wrapper. Other themes default to
   * `standard` так что поведение не меняется.
   */

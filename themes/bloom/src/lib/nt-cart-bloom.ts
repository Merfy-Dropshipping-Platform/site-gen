/**
 * Локальная копия DS `nt-cart.ts` (пакет
 * `@merfy-dropshipping-platform/design-systems-theme`) с поддержкой
 * `variantCombinationId` (Phase 2, спек 098). Пакетная версия не несёт
 * combinationId реальной комбинации, а он нужен для backend cart → order_items.
 *
 * bloom/cart.ts использует только `createNtCart` + типы отсюда (свой
 * initCartUI/drawer-рендер живёт в cart.ts), поэтому здесь важны
 * `makeLineId` (включает combinationId) и `addToCart` (кладёт variant с ним).
 */
...
export const createNtCart = (opts: NtCartCreateOptions) => {
	...
	/**
	 * Базовый drawer-рендер DS (на случай, если тема использует его напрямую).
	 * bloom/cart.ts вешает СВОЙ initCartUI с Bloom-модалкой, поэтому этот метод
	 * там не вызывается — но onClick читает `data-variant-combination-id`, чтобы
	 * локальный API оставался полным аналогом пакетного.
	 */
	const initCartUI = () => {
		if (typeof window === "undefined") return;

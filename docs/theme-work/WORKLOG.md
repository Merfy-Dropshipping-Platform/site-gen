...
- `pnpm conformance:satin` красный — **проверено сборкой чистого `origin/main`
  в отдельном worktree: там ровно то же «tracked inventory is stale»**. Значит
  унаследованное. Ветка тоже сдвигает `sourceDigest` (правила пинят
  `themes/satin/src/pages/checkout.astro`), поэтому перед заливкой нужен
  документированный шаг: коммит → `pnpm conformance:satin:refresh-inventory` →
  коммит.
...
- `origin/main` за время работы ушёл `ed3d5948` → `6d7537b3`. Ветка НЕ слита.

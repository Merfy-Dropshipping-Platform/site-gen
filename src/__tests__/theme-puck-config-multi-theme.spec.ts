...
function loadManifest(
  theme: "rose" | "vanilla" | "bloom" | "satin" | "flux",
): ThemeConfigForResolver {
  const raw = readFileSync(
    resolve(ROOT, "packages", `theme-${theme}`, "theme.json"),
    "utf-8",
  );
...
const BASE_BLOCKS: Record<string, BaseBlockEntry> = Object.fromEntries(
  ["Hero", "Header", "Footer", "AuthModal"].map((name) => [
    name,
...
describe("Theme manifest resolver (Phase 2a multi-theme wiring)", () => {
  ...
  it("vanilla manifest has distinct signature from rose", () => {
    const vanilla = JSON.parse(
      readFileSync(
        resolve(ROOT, "packages", "theme-vanilla", "theme.json"),
        "utf-8",
      ),
    );
    const rose = JSON.parse(
      readFileSync(
        resolve(ROOT, "packages", "theme-rose", "theme.json"),
        "utf-8",
      ),
    );
  ...
  it("bloom manifest inherits Header + Footer from base (spec 089 — no overrides)", () => {
    ...
    const bloomJson = JSON.parse(
      readFileSync(
        resolve(ROOT, "packages", "theme-bloom", "theme.json"),
        "utf-8",
      ),
    );
  ...
  it("satin manifest overrides Header + Footer with monochrome + flat (0px) radii signature", () => {
    ...
    const satinJson = JSON.parse(
      readFileSync(
        resolve(ROOT, "packages", "theme-satin", "theme.json"),
        "utf-8",
      ),
    );
  ...
  it("flux manifest overrides neither Header nor Footer (Task 9, 111-flux-layout-parity: stale incomplete Header override removed)", () => {
    ...
    const fluxJson = JSON.parse(
      readFileSync(
        resolve(ROOT, "packages", "theme-flux", "theme.json"),
        "utf-8",
      ),
    );

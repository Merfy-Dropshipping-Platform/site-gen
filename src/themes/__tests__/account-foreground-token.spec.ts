...
const selected = (css: string, prefix: string): Rule[] =>
  rules(css).filter((r) =>
    r.selector.split(",").some((s) => s.trim().startsWith(prefix)),
  );
...
const schemeTokens = (theme: string): Set<string> => {
  ...
  return new Set(
    [...(rule as RegExpExecArray)[1].matchAll(/(--[a-zA-Z0-9_-]+)\s*:/g)].map(
      (m) => m[1],
    ),
  );
};
...
describe("аккаунт и метки входа — цвет из живого токена схемы", () => {
  it.each(THEMES)(
    ...
    (theme) => {
      const broken = accountRules(globalCss(theme)).filter((r) =>
        r.body.includes(DEAD),
      );
      expect(
        `${theme}: правил .account-* с ${DEAD} = ${broken.length}` +
          (broken.length
            ? ` (${broken.map((r) => r.selector).join(", ")})`
            : ""),
  ...
  it.each(THEMES)(
    "%s: каждая краска аккаунта и метки входа доходит до роли схемы",
    (theme) => {
      const css = globalCss(theme);
      const declared = schemeTokens(theme);
      const watched = [
        ...accountRules(css),
        ...selected(css, ".auth-label"),
        ...selected(css, ".auth-input"),
      ];
      const dead = watched
      ...
      expect(
        `${theme}: красок мимо ролей схемы = ${dead.length}` +
          (dead.length
            ? ` (${dead.map((d) => `${d.selector}: ${d.expr}`).join(" | ")})`
            : ""),
      ).toBe(`${theme}: красок мимо ролей схемы = 0`);
      expect(watched.filter((r) => colorDecl(r.body)).length).toBeGreaterThan(
        5,
      );
    },
  );
  ...
  it.each(THEMES)("%s: заголовок и текст не перепутаны местами", (theme) => {
    ...
    for (const r of watched) {
      ...
      checked++;
      if (heading && !readsHeading)
        wrong.push(`${r.selector}: font-heading, а роль ${expr}`);
      if (body && !readsText)
        wrong.push(`${r.selector}: font-body, а роль ${expr}`);
    }
    expect(
      `${theme}: роль перепутана в ${wrong.length} правилах` +
        (wrong.length ? ` (${wrong.join(" | ")})` : ""),
    ).toBe(`${theme}: роль перепутана в 0 правилах`);
    expect(checked).toBeGreaterThan(3);
  });

  it.each(THEMES)(
    "%s: .auth-label читает роль схемы напрямую, а не через мёртвый токен",
    (theme) => {
      const rs = selected(globalCss(theme), ".auth-label").filter((r) =>
        colorDecl(r.body),
      );
      expect(rs.length).toBeGreaterThan(0);
      const bad = rs.filter((r) =>
        (colorDecl(r.body) as string).includes(DEAD),
      );
      ...
      expect(
        rs.every((r) =>
          (colorDecl(r.body) as string).includes("var(--color-text"),
        ),
      ).toBe(true);
    },
  );
  ...
  it.each(THEMES)(
    ...
    (theme) => {
      const rs = selected(globalCss(theme), ".auth-input").filter((r) =>
        colorDecl(r.body),
      );
      expect(rs.length).toBeGreaterThan(0);
      const withDead = rs.filter((r) =>
        (colorDecl(r.body) as string).includes(DEAD),
      );
      ...
      const withoutFallback = withDead.filter(
        (r) => !(colorDecl(r.body) as string).includes("var(--color-text"),
      );
      expect(
        `${theme}: правил .auth-input с ${DEAD} без фолбэка на --color-text = ${withoutFallback.length}`,
      ).toBe(

...
export class BillingListenerController {
  ...
  async handleSubscriptionUpdated(@Payload() payload: SubscriptionUpdatedPayload, @Ctx() _ctx: RmqContext) {
    try {
      ...
      if (!tenantId) {
        this.logger.warn('billing.subscription.updated: cannot determine tenantId');
        return;
      }
      ...
      const wasFreeze = ['past_due', 'frozen', 'canceled'].includes(previousStatus);
      ...
      if (payload?.shopsLimit !== undefined) {
        await this.checkSitesLimit(tenantId, payload.shopsLimit, payload.planName);
      }
    } catch (e) {
   ...
   */
  private async checkSitesLimit(tenantId: string, newLimit: number, planName?: string) {
    try {
      ...
      this.logger.warn(`Failed to check sites limit for tenant ${tenantId}: ${e instanceof Error ? e.message : e}`);
    }
  }
}

/**
 * Локальная копия DS `nt-cart.ts` для темы vanilla.
 *
 * Зачем копия, а не пакет `@merfy-dropshipping-platform/design-systems-theme/lib/nt-cart`:
 * нужен `variantCombinationId` в линии корзины (Phase 2, спек 098) — реальный
 * combinationId уходит в backend cart → order_items при оформлении. Пакет
 * отдаёт variant только { color, size }. Менять пакет нельзя (общий для тем).
 */

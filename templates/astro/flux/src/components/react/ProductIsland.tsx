...
/**
 * ProductIsland — React island for the flux theme product detail page (Puck-managed).
 *
 ...
 *
 * Visual design matches the pre-T15 flux product page:
 *   - 1320px container (max-w-[1320px])
 *   - photo LEFT, info RIGHT (photoPosition default: 'left')
 *   - gallery column 552px wide with `flex-row-reverse`: thumbnails (88px vertical strip)
 *     appear visually LEFT of the main image
 *   - main image: aspect-square, --color-foreground/0.03 background, no radius on container
 *   - typography: 14px brand, 24px / 1.366 product name (NO uppercase — Roboto Flex sans),
 *     16px price, 14px line-through old price
 *   - 56px buttons, 16/22 font, --radius-button (= 6px in flux)
 *   - solid 'Buy Now' uses --color-button / --color-button-text tokens
 *     (flux: orange #fa5109 with white text — the signature accent of the techy palette)
 *   - thumbnail border-radius uses --radius-card (= 12px in flux)
 *   - 40px gap between major sections
 *
 * NOTE: tokens that change visuals between themes:
 *   - --radius-button (flux 6px vs satin 0px vs vanilla 0px)
 *   - --radius-card    (flux 12px vs satin 0px vs vanilla 0px)
 *   - --color-button   (flux #fa5109 orange accent vs vanilla olive vs satin black)
 *   - --color-muted    (flux #cccccc lighter vs satin/vanilla darker)
 * Identical layout/typography to satin/vanilla — flux's distinctive look comes from tokens.
 */
...
const ProductIsland: React.FC<ProductIslandProps> = (props) => {
  const {
    productId,
    // Flux default: photo on LEFT, info on RIGHT — matches pre-T15 layout
    // (see git show f90b1e8^:templates/astro/flux/src/pages/product/[id].astro:
    // outer `flex flex-col lg:flex-row gap-[40px]` with gallery rendered first).
    photoPosition = 'left',
    text,
    title,
    price: priceProp,
  ...
  // Flux default: photo LEFT, info RIGHT — matches pre-T15 layout.
  // Default ordering: gallery first, info second; lg:flex-row places gallery LEFT,
  // info RIGHT. To put the photo on the RIGHT, reverse the row.
  const flexDir =
    ...
    >
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 py-[75px]">
        <div className={`flex flex-col ${flexDir} gap-[40px]`}>
          {/* --- Image gallery column (LEFT in default layout, 552px on desktop) ---
              Inner layout: flex-row-reverse — children render in DOM as
              [main image, thumbnails], but row-reverse flips visual order so
              thumbnails appear LEFT of the main image (matches pre-T15 flux). */}
          <div className="w-full lg:w-[552px] lg:shrink-0 flex flex-row-reverse" style={{ gap: 8 }}>
            {/* Main image — aspect-square, --color-foreground/0.03 bg, no radius on container */}
            <div
              className="overflow-hidden flex-1 aspect-square flex items-center justify-center"
              style={{
            ...
            {/* Thumbnails — vertical strip 88px wide, hidden on mobile.
                Uses --radius-card (= 12px in flux) for soft rounded squares
                that match flux's card radius scale. */}
            {hasMultipleImages ? (
              <div
          ...
          {/* --- Product info column (RIGHT in default layout, lg:flex-1) --- */}
          <div className="flex flex-col w-full lg:flex-1" style={{ gap: 40 }}>
            <div className="flex flex-col">
              ...
              {/* Brand name — 14px, 1.4, color-muted */}
              {brandName ? (
                <span
                  className="font-body"
                  style={{
              ...
              {/* Product name — flux: 24px, line-height 1.366, NO uppercase
                  (Roboto Flex sans provides the modern techy look). */}
              {showTitle ? (
                <h1
                  className="font-body"
                  style={{
              ...
              {/* Price — flux: 16px primary, 14px line-through old price, 8px gap */}
              {showPrice ? (
                <div
            ...
            {/* Cart buttons — flux: 56px, 16/22 font, 12px gap, button color tokens.
                'Buy Now' uses --color-button (#fa5109 orange) / --color-button-text (#fff)
                — flux's signature accent that drives conversions on tech stores. */}
            ...
            {/* Description — flux: h2 20px UPPERCASE, p 16/22 color-muted */}
            {showDescription &&

...
declare global {
  interface Window {
    __bloomAuth?: typeof bloomAuth;
    __bloomAuthBound?: boolean;
  }
}

export const bloomAuth = {
  // сторы
  $token,
  $customer,
  $authStatus,
...
/**
 * initAuthUI — выставляет window.__bloomAuth (для inline-скриптов карточек/хедера),
...
export function initAuthUI(): void {
  ...
  if (!window.__bloomAuthBound) {
    window.__bloomAuthBound = true;

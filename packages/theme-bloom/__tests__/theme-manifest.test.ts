...
describe('@merfy/theme-bloom theme.json', () => {
  ...
  it('defaults express bloom signature: 1320px container + pill (100px) buttons', () => {
    expect(manifest.defaults['--container-max-width']).toBe('1320px');
    expect(manifest.defaults['--radius-button']).toBe('100px');
    expect(manifest.defaults['--radius-card']).toBe('12px');
    expect(manifest.defaults['--radius-input']).toBe('4px');
  });

  it('declares Urbanist + Inter fonts', () => {
    const families = manifest.fonts.map((f: { family: string }) => f.family);
    expect(families).toContain('Urbanist');

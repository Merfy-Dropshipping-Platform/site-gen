...
export const HeaderClasses = {
  ...
  /**
   * Additive variant `'rose-classic'` — pixel-perfect port of legacy
   * rose.merfy.ru showcase Header.
   *
   * Differs from base / standard variant:
   *   • Wider max-width (1920px) + responsive padding ступени до 2xl:px-[280px]
   *   • Larger nav gaps up to 2xl:gap-[80px]
   *   • Heavier shadow wrapper (`shadow-sm`)
   *   • Comfortaa logo style (text-[20px] font-normal uppercase tracking-normal,
   *     color-primary через CSS-var)
   *   • Tighter logoImg responsive heights (h-5/h-6/h-[26px])
   *   • Actions gap up to 2xl:gap-[25px]
   *
   * Used by `variants/RoseClassic.astro` (sub-component selected by base
   * Header.astro when `variant === 'rose-classic'`). Inline SVG iconography
   * остаётся в `RoseClassic.astro` 1:1 (без token-driven mask-image, user
   * explicit: logo + SVG грузятся через интерфейс конструктора).
   */
  variant: {
    'rose-classic': {
      wrapper:
        'w-full shadow-sm bg-[rgb(var(--color-bg))] text-[rgb(var(--color-text))]',
      sticky: {
        'scroll-up': 'sticky top-0 z-50 transition-transform duration-300',
        always: 'sticky top-0 z-50',
        none: 'relative z-50',
      ...
      header:
        'w-full flex items-center border-b border-[rgb(var(--color-text))]/15',
      ...
      logoText:
        'text-[20px] font-normal uppercase tracking-normal leading-none [font-family:var(--font-heading)] text-[rgb(var(--color-primary))]',
      navMenu: 'hidden md:flex items-center gap-4 lg:gap-8 xl:gap-12 2xl:gap-[80px]',
      navMenuCentered:
        'hidden md:flex items-center justify-center gap-4 lg:gap-8 xl:gap-12 2xl:gap-[80px] mt-2',
      navLink:
        'text-sm md:text-base lg:text-lg xl:text-[20px] font-normal hover:opacity-70 transition-opacity [font-family:var(--font-body)] text-[length:var(--size-nav-link)] text-[rgb(var(--color-text))]',
      actions:
        'flex items-center gap-3 sm:gap-3 md:gap-4 lg:gap-5 xl:gap-[25px]',
      actionSearch:
        'hidden md:flex w-8 h-8 lg:w-10 lg:h-10 items-center justify-center hover:opacity-70 transition-opacity text-[rgb(var(--color-text))]',
      actionCart:
        'relative w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex items-center justify-center hover:opacity-70 transition-opacity text-[rgb(var(--color-text))]',
      ...
      mobileMenu: {
        root:
          'hidden md:hidden absolute top-full left-0 right-0 border-b border-[rgb(var(--color-text))]/15 shadow-lg z-50 bg-[rgb(var(--color-bg))]',
        nav: 'flex flex-col',
        search: 'px-4 pt-6 pb-4',
        searchInput:
          'block w-full h-12 pl-5 pr-12 border border-[rgb(var(--color-text))]/30 rounded-lg text-sm font-normal outline-none [font-family:var(--font-body)] bg-[rgb(var(--color-bg))] text-[rgb(var(--color-text))] placeholder:text-[rgb(var(--color-muted))]',

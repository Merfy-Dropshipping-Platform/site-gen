...
export async function buildScaffold(
  ...
  if (config.dynamicPages) {
    ...
    // 084 Stage 3 — uniform /catalog/[slug] route across all themes.
    // Generates SEO-friendly per-collection catalog page if not present.
    const catalogSlugPath = path.join(
      outputDir,
      "src",
      "pages",
      "catalog",
    ...
    if (!(await fileExists(catalogSlugPath))) {
      const catalogPage = generateCatalogSlugPage({
        ...config.dynamicPages,
        layoutImport: config.layout?.importPath
          ? `../../layouts/${path.basename(config.layout.importPath)}`
          : undefined,
    ...
    // 084 Stage 3 — replace collections/[slug].astro with 301 redirect.
    // Safety: only replace if file is the generic auto-generated one
    // (i.e. doesn't exist OR was generated by us in a prior build —
    // detect via marker comment). Themes shipping their own custom
    // collections/[slug].astro keep theirs untouched.
    ...
    const REDIRECT_MARKER = "/* 084-stage3-uniform-redirect */";
    let shouldWriteRedirect = !(await fileExists(collectionsSlugPath));
    if (await fileExists(collectionsSlugPath)) {
      const existing = await fs.readFile(collectionsSlugPath, "utf8");
      // Already our redirect — overwrite to keep marker fresh
      if (existing.includes(REDIRECT_MARKER)) shouldWriteRedirect = true;
    }
    if (shouldWriteRedirect) {
      const redirectAstro = `---
${REDIRECT_MARKER}
const { slug } = Astro.params;
return Astro.redirect(\`/catalog/\${slug}\`, 301);
---`;
      await writeFile(collectionsSlugPath, redirectAstro);
      generatedFiles.push("src/pages/collections/[slug].astro (redirect)");
    }
  }

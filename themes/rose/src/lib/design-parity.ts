...
/**
 * Карточки «Популярных» под признаком: зазор фото–подпись 12 → 20, цена
 ...
 * долго): только под признаком и только когда значение НЕ задано. Заданное
 * мерчантом значение и рендер без признака получают пустую строку — прежняя
 * разметка байт в байт. Для поля С значением по умолчанию не годится: панель
 * покажет своё значение, а витрина — вид верстальщиков.
 */

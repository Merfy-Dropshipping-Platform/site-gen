 ...
 *
 * Порт rose-эталона (themes/rose/src/lib/wishlist.ts). Контракт data-атрибутов
 * (`[data-wishlist-toggle][data-product-id]`, `[data-wishlist-count]`) общий; ключ
 * localStorage, имя события и глобал зеркалятся с ПРЕФИКСОМ темы (`bloom:`).
 *
 * Дизайн под bloom: сердце берёт геометрию того же heart-path (как rose
 * public/icons/favourite.svg), но рендерится ИНЛАЙН-SVG (а не <img> как NtIcon) —
 * иначе нельзя переключать fill (пусто ↔ залито) и красить в кнопочный токен bloom
 * (--color-button-bg, дефолт розовый #e38e9f). У bloom нет своей favourite.svg —
 * используем тот же универсальный heart-path.
 */

// ── Контракт хранения (per-theme; префикс bloom) ──
const STORAGE_KEY = "bloom:wishlist:v1";
const EVT_UPDATED = "bloom:wishlist:updated";
...
declare global {
	interface Window {
		__bloomWishlist?: BloomWishlistApi;
		__bloomWishlistBound?: boolean;
	}
}

export interface BloomWishlistApi {
	getAll(): string[];
...
/**
 * Инлайн-SVG сердца bloom. Геометрия = универсальный heart-path (тот же, что rose
 * public/icons/favourite.svg, viewBox 0 0 17.4 15.4). filled → залито кнопочным
 * токеном bloom (--color-button-bg, дефолт розовый 227 142 159); outline → только
...
const heartSvg = (filled: boolean): string => {
	const fill = filled ? "rgb(var(--color-button-bg,227 142 159))" : "none";
	const stroke = filled ? "rgb(var(--color-button-bg,227 142 159))" : "currentColor";
	return (
...
export const wishlist: BloomWishlistApi = {
	getAll,
	has,
	add,
	remove,
 ...
 *  • на событие bloom:wishlist:updated перерисовывает ВСЕ кнопки-сердца
 *    (aria-pressed + filled/outline) и ВСЕ бейджи [data-wishlist-count];
 *  • выставляет window.__bloomWishlist для inline-скриптов карточки/PDP.
 *
...
export const initWishlistUI = (): void => {
	if (typeof window === "undefined") return;
	window.__bloomWishlist = wishlist;
	...
	if (!window.__bloomWishlistBound) {
		window.__bloomWishlistBound = true;
		...
		document.addEventListener("click", (event: MouseEvent) => {
			...
			const id = btn.dataset.productId ?? "";
			if (!id) return;
			const nowActive = toggle(id); // событие bloom:wishlist:updated → renderAll ниже

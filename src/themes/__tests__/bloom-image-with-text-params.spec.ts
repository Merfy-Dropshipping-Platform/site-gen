...
describe("«Изображение с текстом» (bloom) — инвентарь параметров", () => {
  ...
  // ── Заглушки и скрытие ────────────────────────────────────────────────
  // 26.09 (владелец, bloom «Коллекция товаров»): мерчант стирает заголовок
  // секции руками — на превью и витрине не должен возвращаться текст темы.
  // Раньше здесь стояло обратное ожидание («пустая строка = не задано →
  // заглушка»): героя-баг был именно в этом — порт не отличал «мерчант стёр»
  // от «поля вовсе нет», и `heading: {text: ""}` подставлял литерал
  // верстальщика назад. Теперь пустая СТРОКА панели рендерится как есть —
  // заголовок не рисуется вовсе (`<h2>` отсутствует, не пустой тег), а
  // заглушка `heading: {text: "Изображение с текстом"}` остаётся только для
  // поля, которого нет вовсе (undefined — старые ревизии).
  it("Пустой заголовок = мерчант стёр вручную → рендерится пусто, без <h2> и без заглушки темы", () => {
    if (!built) return;
    const html = render({ heading: { text: "" } });
    expect(html).not.toContain("Изображение с текстом");
    expect(html).not.toContain("<h2");
  });

  it("Заголовок не задан вовсе (undefined, старая ревизия) → заглушка темы", () => {
    if (!built) return;
    const html = render({ heading: undefined });

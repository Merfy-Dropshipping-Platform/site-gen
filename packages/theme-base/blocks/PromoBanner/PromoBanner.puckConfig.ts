...
export const PromoBannerPuckConfig = {
  ...
  fields: {
    ...
    // «Регистр текста» контролом в панели БЫТЬ НЕ ДОЛЖНО (владелец, 2026-09-13:
    // «такого поля отродясь не было»). Поле завели видимым 2026-05-04
    // (d37adfbe), убрали с экрана 2026-05-16 (2f5c5212) — и так оно стояло почти
    // четыре месяца, пока 2026-09-09 (63a17c25) мы не вернули его селектом.
    // Возвращаем прежнее состояние.
    //
    // `type: 'hidden'`, а не `hiddenInMainPanel`: второе лишь переносит контрол
    // из основной панели в подпанель дерева (CustomFieldsPanel.tsx), а первое
    // возвращает null И там, И в FieldRenderer.tsx — контрола нет нигде. Проп
    // при этом ОСТАЁТСЯ: он есть в схеме ниже и в theme.json
    // (`blockDefaults.PromoBanner.textTransform`), поэтому ревизия, где мерчант
    // уже выбрал регистр, разбирается и рисуется как прежде. Замер рендером по
    // пяти темам: без пропа и при 'none' капса нет, при 'uppercase' он есть.
    textTransform: { type: 'hidden', label: '' } as any,
  },
  defaults: {

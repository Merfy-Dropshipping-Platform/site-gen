export const CartDrawerClasses = {
  ...
  empty: 'font-[var(--font-body)] text-[rgb(var(--color-text))] opacity-60 text-center py-8',
  footer: 'px-4 py-3 border-t border-[rgb(var(--color-bg))] flex flex-col gap-3',
  total: 'flex items-center justify-between font-[var(--font-body)] text-[rgb(var(--color-text))]',
  checkoutBtn: 'h-[var(--size-hero-button-h)] rounded-[var(--radius-button)] bg-[rgb(var(--color-button-bg))] text-[rgb(var(--color-button-text))] font-[var(--font-body)] text-center flex items-center justify-center no-underline',
} as const;

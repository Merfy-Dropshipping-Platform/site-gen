...
export const ImageWithTextPuckConfig: BlockPuckConfig<ImageWithTextProps> = {
  ...
  fields: {
    ...
    // Подпись менялась дважды, обе — решения владельца:
    //   17.09 (b79) «Раскладка» → «Контейнер», как у MultiColumns/MultiRows/
    //         CollapsibleSection;
    //   22.09 «не контейнер, а наложение» — при разборе схемы секции стало
    //         видно, что у соседей «Контейнер» включает ПОДЛОЖКУ, а здесь
    //         тумблер включает НАЕЗД текста на фото. Это разные вещи под
    //         одним именем, и они же были источником путаницы.
    // Теперь «Наложение» — режим, а «контейнер» — сама плашка под текстом
    // (её по-прежнему красит «Цветовая схема контейнера»).
    // Ключ пропа `containerEnabled` НЕ переименован: он лежит в сохранённых
    // ревизиях, и порт читает именно его. Меняется только подпись.
    containerEnabled: {
      type: 'toggle',
      label: 'Наложение',
      options: [

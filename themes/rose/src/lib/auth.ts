...
declare global {
  interface Window {
    __roseAuth?: typeof roseAuth;
    __roseAuthBound?: boolean;
  }
}

export const roseAuth = {
  // сторы
  $token,
  $customer,
  $authStatus,
...
/**
 * initAuthUI — выставляет window.__roseAuth (для inline-скриптов карточек/хедера),
...
export function initAuthUI(): void {
  ...
  if (!window.__roseAuthBound) {
    window.__roseAuthBound = true;

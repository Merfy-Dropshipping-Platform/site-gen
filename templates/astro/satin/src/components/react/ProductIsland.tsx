...
/**
 * ProductIsland — React island for the satin theme product detail page (Puck-managed).
 *
 ...
 *
 * Visual design matches the pre-T14 satin product page:
 ...
 *   - typography: 14px brand, 24px / 1.366 product name (NO uppercase — Kelly Slab serif),
 *     16px price, 14px line-through old price
 *   - 56px buttons, 16/22 font, --radius-button (= 0px in satin)
 *   - solid 'Buy Now' uses --color-button / --color-button-text tokens
 *   - all radii are 0 (satin design language); --radius-button / --radius-card tokens applied
...
const ProductIsland: React.FC<ProductIslandProps> = (props) => {
  const {
    productId,
    // Satin default: photo on LEFT, info on RIGHT — matches pre-T14 layout.
    photoPosition = 'left',
    text,
    title,
    price: priceProp,
  ...
  // Satin default: photo LEFT, info RIGHT — matches pre-T14 layout.
      ...
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 py-[75px]">
        <div className={`flex flex-col ${flexDir} gap-[40px]`}>
          {/* --- Image gallery column (LEFT in default layout, 552px on desktop) ---
              ...
              thumbnails appear LEFT of the main image (matches pre-T14 satin). */}
          ...
          <div className="flex flex-col w-full lg:flex-1" style={{ gap: 40 }}>
            <div className="flex flex-col">
              ...
              {/* Product name — satin: 24px, line-height 1.366, NO uppercase
                  (Kelly Slab serif provides the elegant fashion look). */}
              {showTitle ? (
                <h1
                  className="font-body"
                  style={{
              ...
              {/* Price — satin: 16px primary, 14px line-through old price, 8px gap */}
              {showPrice ? (
                <div
            ...
            {/* Cart buttons — satin: 56px, 16/22 font, 12px gap, button color tokens */}
            ...
            {/* Description — satin: h2 20px UPPERCASE, p 16/22 color-muted */}
            {showDescription &&

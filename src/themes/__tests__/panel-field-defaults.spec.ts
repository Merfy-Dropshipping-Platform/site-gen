...
const CONTENT_TYPES: Record<string, string> = {
  ...
  publicationPicker: "выбор публикации мерчанта",
...
const FIELD_EXCEPTIONS: Record<string, FieldException> = {
  ...
  // ── «Размер текста» «Контактной формы»: дефолта НЕТ намеренно ─────────────
  // Поле добавлено по просьбе владельца 2026-09-16 («под инпутом добавить
  // список размер текста»). Дефолт здесь ВРЕДЕН: панель вписывает его в секцию
  // при ЛЮБОЙ правке (см. updateProp), а средняя ступень у каждой темы
  // подобрана равной прежнему жёсткому размеру абзаца — rose 12px, остальные
  // 16px, flux 14/16. Со статичным дефолтом секции тех мерчантов, кто размер не
  // выбирал, поехали бы от самой правки. Отсутствие значения = «размер темы»,
  // и панель не пустая: SelectField показывает «Средний».
  "ContactForm.textSize": {
    reason:
      "дефолта нет намеренно: средняя ступень каждой темы равна прежнему размеру абзаца, статика сдвинула бы вид секций, где мерчант размер не выбирал",
  },
  ...
  // ── Отступы секции: фолбэк порта — АДАПТИВНАЯ лесенка, не пара чисел ─────
  // Порт каждой темы держит свой вертикальный ритм классами и меняет его по
  // брейкпоинтам: rose pt-14 → md:100 → lg:120 → xl:140, vanilla py-20 →
  // md:py-28 (80→112), flux py-10 → md:py-16 (40→64), satin py-8 → md:py-12
  // (32→48), bloom py-20 → md:py-[120px]. Пара {top,bottom} этого не выражает,
  // а инлайн-стиль, который из неё рождается, лесенку ПЕРЕБИВАЕТ (замер
  // рендером: с пропом `style="padding-top:80px"`, без пропа стиля нет). Любой
  // статичный дефолт здесь сплющил бы все пять тем в одно число — ровно это и
  // делали прежние 80/80. Тему, которой нужен свой отступ, обслуживает
  // theme.json `blockDefaults` (он доезжает и до витрины, не только до панели).
  "*.padding": {
    reason:
      "фолбэк порта — адаптивная лесенка классов (rose 56→140px, vanilla 80→112, flux 40→64, satin 32→56, bloom 80→120); пара чисел её не выражает, а инлайн-стиль из неё лесенку перебивает",
  },

...
## 2026-09-23 — подсветка совпадения в выдаче поиска (первая ступень поиска)

Ветка `feat/header-search-highlight`, worktree `.worktrees/search-highlight`.
Документ «Продвинутый LIKE для витрины», первая ступень; site-gen — только
подсветка в выдаче, product-service и gateway получают своё сопоставление по
словам параллельно, в этой ветке не трогались.

- Общий модуль `packages/theme-base/runtime/header-search.ts`: новый
  `highlightMatch(text, query)` + `ctx.highlight` в `HeaderSearchRenderContext`.
  Конвейер маленьких чистых функций: нормализовать посимвольно (нижний
  регистр, ё→е, небуквенное → пробел, БЕЗ схлопывания и обрезки — длина
  строки не меняется, иначе съедут индексы совпадения) → найти отрезки
  совпадений по каждому слову запроса (правило коротких слов — отдельный
  предикат `isAllowedMatch`/`isShortWord`/`isAtWordStart`) → склеить
  пересекающиеся и смежные отрезки в один → отрисовать экранированный HTML,
  оборачивая найденные отрезки в `<mark data-search-mark
  style="background:none;color:inherit;font-weight:700">`.
- Слово из 1–2 букв подсвечивается только с начала слова («кр» в «Кроссовки
  Runner» — да, «ра» в середине «красный» — нет); от 3 букв — где угодно
  в слове. Нормализация совпадает с серверной (та же формула ё→е и
  «небуквенное — пробел»), но применяется посимвольно, а не с схлопыванием
  пробелов — сервер это может себе позволить (не важна позиция), подсветке
  позиция как раз и нужна.
- Во всех пяти `themes/<t>/src/lib/header-search-view.ts` название товара в
  текстовом узле переведено с `ctx.escapeHtml` на `ctx.highlight` (rose и
  bloom — два места, panel и drawer; vanilla/satin/flux — одно общее для
  обеих раскладок). Атрибуты не тронуты: у bloom `data-name` кнопки
  «В корзину» остался на `ctx.escapeHtml` — вставить туда `<mark>` сломало бы
  значение атрибута.
- Тесты: `pnpm test:header-search` было 77, стало 112 (+8 в рантайм-тесте —
  сама функция: регистр, ё/е в обе стороны, короткие слова, пересечения,
  несколько слов, XSS, пустой запрос, плюс подключение `ctx.highlight` через
  `initHeaderSearch`; +26 в тестах портов — по всем пяти темам и обеим
  раскладкам: название подсвечено, короткое слово не подсвечено в середине,
  bloom `data-name` не подсвечен, экранирование не ломается даже когда
  запрос совпадает с частью вредоносного названия у `<img>`).
- Саботаж 3 из 3 — каждый красит СВОЮ проверку, версия возвращена: убрал
  правило «с начала слова» → 11 проверок красные; убрал замену ё→е → 1
  красная; убрал экранирование в `<mark>`/остатке текста → 17 красных (в том
  числе старый тест XSS, существовавший до этой правки).
- Стиль (требование владельца, пришло в процессе): подсветка — декларативный
  конвейер маленьких именованных функций, без if/else-лестниц и тернарников,
  вложенность не глубже двух уровней, длинные цепочки (`.map().split().join()`
  и т.п.) разбиты на именованные шаги. Правило коротких слов — один предикат
  `isAllowedMatch`, а не ветвление по месту вызова.
- Сборка по цепочке CI (`build`, `build:blocks`, `build:theme-sections:all`,
  `build:preview-tailwind`) прошла зелёной.
- ХВОСТ (не мой): `pnpm test:section-snapshots` (8 из 155) и
  `pnpm conformance:satin` падают на четырёх блоках satin, которых подсветка
  не касалась — Newsletter, CollapsibleSection, MultiColumns, Publications;
  причина — сторонний пакет `@merfy-dropshipping-platform/design-systems-theme`
  (`NtIcon.astro`) не грузится напрямую Node/tsx («Unknown file extension
  ".astro"»). Проверено на чистом срезе того же коммита origin/main (та же
  одолженная у родительской папки `node_modules`) — там все 155 снимков и
  conformance:satin зелёные, значит не в правке дело и не в версии пакета.
  Перепробовано без результата: порядок сборки как в CI, отдельный
  `run-theme-build.ts satin`, локальный симлинк `node_modules`, чистая
  пересборка `dist/theme-sections/satin` — во всех случаях в этой worktree
  падает одинаково и детерминированно. Нужна проверка в настоящем CI
  (свежий `pnpm install`, не одолженный).

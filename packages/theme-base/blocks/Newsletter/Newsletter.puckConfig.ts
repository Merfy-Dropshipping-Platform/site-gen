...
export const NewsletterPuckConfig: BlockPuckConfig<NewsletterProps> = {
  ...
  defaults: {
    ...
    // Дефолт СОВПАДАЕТ с тем, что рисуют порты всех пяти тем (дословный
    // плейсхолдер Figma 1:19335). Раньше здесь стоял текст верстальщика
    // «Узнавай о новинках и акциях первым», а порты его заменили — и панель
    // с витриной расходились ДАЖЕ БЕЗ ПРАВОК мерчанта (баг тестера 18.09:
    // «в панели одно, на витрине другое»). Поле скрытое, легаси-фолбэк:
    // видимый контрол — «Текст» (text.content).
    description: 'Узнавайте первыми о новых коллекциях и эксклюзивных предложениях.',

 ...
 *    fallback). Используется drawer-корзиной в шапке (nt-cart-flux.ts initCartUI).
 *
...
function thumbHtml(line: CartLineExtra): string {
	...
	return `
		<a href="/products/${encodeURIComponent(line.productId)}" class="relative block size-full overflow-hidden rounded-[8px] bg-[#F5F5F5]">
			<picture class="contents">
				${noWebp ? "" : `<source srcset="${webpAbs}" type="image/webp" />`}
				...
				<div class="flex min-w-0 flex-1 flex-col gap-4">
					<div class="flex flex-col gap-1">
						...
						<div class="flex items-center gap-2 uppercase">
							...
							${oldPrice ? `<span class="font-roboto-flex text-[16px] font-normal leading-normal text-[rgb(var(--color-muted,204_204_204))] line-through">${oldPrice}</span>` : ""}
						</div>
					</div>
					${
						color || size

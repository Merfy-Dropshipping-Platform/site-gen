...
const NO_NAMED_FIELDS: Record<string, string> = {
  ...
  OrdersSection:
    "нет в NAMED_SUBSECTIONS: в панели только цветовая схема (канон)",
  // «Вход»: в панели РОВНО ЧЕТЫРЕ параметра — «Заголовок», «Текст», «Цветовая
  // схема», «Отступы» (канон, владелец 14.09). «Заголовок» и «Текст» — простые
  // поля ввода, а НЕ именованные подсекции с «глазом»: скрывать их поштучно
  // владелец не просил, и в NAMED_SUBSECTIONS конструктора их нет. Пустой
  // «Текст» просто не рисует абзац — это дефолт поля, а не «глаз».
  // Состав сторожат panel-canon + login-section.spec.ts.
  LoginSection:
    "нет в NAMED_SUBSECTIONS: заголовок и текст — простые поля, не подсекции",
};

...
/**
 * ProductIsland — React island for the bloom theme product detail page (Puck-managed).
 *
 ...
 *
 * Visual design matches the pre-T16 bloom product page:
 ...
 *   - main image: aspect-square, --color-foreground/0.03 background, no radius
 *     (lets the pink theme shine through whitespace)
 *   - thumbs: 88×88 with --radius-card (12px) corners — soft feminine vibe
 *   - typography: 14px brand (color-muted), 24px / 1.366 product name (Urbanist heading
 *     via theme font stack), 16px price, 14px line-through old price
 *   - 56px buttons with --radius-button (100px = pill in bloom) — matches CTA pill design
 *   - solid 'Buy Now' uses --color-button token = #CF7A8B pink — bloom's signature accent
 *   - description heading uppercase, 20/27, color-foreground; body 16/22 color-muted
 *   - 40px gap between major sections, 25px gap inside variant blocks
 */
...
const ProductIsland: React.FC<ProductIslandProps> = (props) => {
  const {
    productId,
    // Bloom default: photo on LEFT, info on RIGHT — matches pre-T16 layout.
    photoPosition = 'left',
    text,
    title,
    price: priceProp,
  ...
  // Bloom default: photo LEFT, info RIGHT — matches pre-T16 layout.
  ...
  // In bloom, --radius-button = 100px → variant pills become fully rounded "stadium" shapes.
      ...
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 py-[75px]">
        <div className={`flex flex-col ${flexDir} gap-[40px]`}>
          {/* --- Image gallery column (LEFT in default layout, 552px on desktop) ---
              ...
              thumbnails appear LEFT of the main image (matches pre-T16 bloom). */}
          <div className="w-full lg:w-[552px] lg:shrink-0 flex flex-row-reverse" style={{ gap: 8 }}>
            {/* Main image — aspect-square, --color-foreground/0.03 bg, no radius */}
            <div
            ...
            {/* Thumbnails — vertical strip 88px wide, hidden on mobile.
                Bloom: --radius-card (12px) gives soft rounded corners on thumbs. */}
            {hasMultipleImages ? (
              <div
          ...
          <div className="flex flex-col w-full lg:flex-1" style={{ gap: 40 }}>
            <div className="flex flex-col">
              ...
              {/* Product name — bloom: 24px, line-height 1.366, NO uppercase
                  (Urbanist heading + Inter body provide the elegant beauty look). */}
              {showTitle ? (
                <h1
                  className="font-body"
                  style={{
              ...
              {/* Price — bloom: 16px primary, 14px line-through old price, 8px gap */}
              {showPrice ? (
                <div
            ...
            {/* Cart buttons — bloom: 56px, 16/22 font, 12px gap.
                --radius-button (100px) makes both into stadium pills.
                Solid 'Buy Now' uses --color-button (#CF7A8B pink) — bloom signature. */}
            ...
            {/* Description — bloom: h2 20px UPPERCASE, p 16/22 color-muted */}
            {showDescription &&
